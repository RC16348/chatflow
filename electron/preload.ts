...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  dbPath: {
    ...
    onAutoDetectProgress: (callback: (progress: { stage: string; currentPath: string; scannedCount: number; foundCount: number; percentage: number }) => void) => {
      const listener = (_: any, progress: any) => callback(progress)
      ipcRenderer.on('dbpath:autoDetect:progress', listener)
      return () => ipcRenderer.removeListener('dbpath:autoDetect:progress', listener)
    }
  },
  ...
  chat: {
    ...
    testAntiRevokeNotification: () =>
      ipcRenderer.invoke('chat:testAntiRevokeNotification'),
    checkSessionForRevoke: (sessionId: string, summary: string, timestamp: number, displayName: string) =>
      ipcRenderer.invoke('chat:checkSessionForRevoke', sessionId, summary, timestamp, displayName),
    ...
    getChatDates: (sessionIds: string[]) => ipcRenderer.invoke('chat:getChatDates', sessionIds),
    ...
    getVoiceTranscript: (sessionId: string, msgId: string, createTime?: number, senderWxid?: string) => ipcRenderer.invoke('chat:getVoiceTranscript', sessionId, msgId, createTime, senderWxid),
    ...
    getContactGender: (username: string) => ipcRenderer.invoke('chat:getContactGender', username),
    getContactsGender: (usernames: string[]) => ipcRenderer.invoke('chat:getContactsGender', usernames),
    getAllFriendsGenderDistribution: () => ipcRenderer.invoke('chat:getAllFriendsGenderDistribution'),
    getMyInfo: () => ipcRenderer.invoke('chat:getMyInfo'),
    getCommonGroupCount: (myDisplayName: string, friendUsername: string) => ipcRenderer.invoke('chat:getCommonGroupCount', myDisplayName, friendUsername),
  ...
  // 云端授权验证
  licenseAuth: {
    verify: () => ipcRenderer.invoke('licenseAuth:verify'),
    getMachineUUID: () => ipcRenderer.invoke('licenseAuth:getMachineUUID'),
    copyToClipboard: (text: string) => ipcRenderer.invoke('licenseAuth:copyToClipboard', text),
    quitApp: () => ipcRenderer.invoke('licenseAuth:quitApp'),
    retryVerify: () => ipcRenderer.invoke('licenseAuth:retryVerify'),
    fetchApiKey: () => ipcRenderer.invoke('licenseAuth:fetchApiKey'),
    clearApiKeyCache: () => ipcRenderer.invoke('licenseAuth:clearApiKeyCache')
  },

  // Agent 对话存储
  agent: {
    listConversations: () => ipcRenderer.invoke('agent:listConversations'),
    getConversation: (convId: string) => ipcRenderer.invoke('agent:getConversation', convId),
    createConversation: (convId: string, title: string) => ipcRenderer.invoke('agent:createConversation', convId, title),
    saveMessage: (convId: string, message: any) => ipcRenderer.invoke('agent:saveMessage', convId, message),
    deleteConversation: (convId: string) => ipcRenderer.invoke('agent:deleteConversation', convId),
    clearAllData: () => ipcRenderer.invoke('agent:clearAllData'),
    executeTool: (toolName: string, args: any) => ipcRenderer.invoke('agent:executeTool', toolName, args),
    appendAgentLog: (sessionId: string, content: string) => ipcRenderer.invoke('agent:appendAgentLog', sessionId, content)
  }
})
