...
function appendLibrarySearchPath(libDir: string): void {
    ...
    if (process.platform === 'win32') {
        ...
        const paths = current.split(';').filter(Boolean)
        if (!paths.includes(libDir)) {
            process.env.PATH = [libDir, ...paths].join(';')
        }
    }
}
...
function prepareSherpaRuntimeEnv(): void {
    ...
    // 从 NODE_PATH 中获取 node_modules 路径
    const separator = process.platform === 'win32' ? ';' : ':'
    const nodePaths = (process.env.NODE_PATH || '').split(separator).filter(Boolean)

    // 构建平台包的可能路径（从 NODE_PATH 推断）
    const candidates: string[] = []
    for (const nodeModulesPath of nodePaths) {
        const platformPath = join(nodeModulesPath, platformPkg)
        if (existsSync(platformPath)) {
            candidates.push(platformPath)
        }
    }
    ...
    for (const dir of fallbackCandidates) {
        if (existsSync(dir) && !candidates.includes(dir)) {
            candidates.push(dir)
        }
    }
    ...
    // 将 node_modules 路径添加到 module.paths 前面
    for (const nodeModulesPath of nodePaths) {
