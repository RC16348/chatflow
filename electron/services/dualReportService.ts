...
class DualReportService {
      ...
      if (data.length >= 4) {
        ...
        if (magicLE === 0xFD2FB528 || magicBE === 0xFD2FB528) {
          ...
          try {
            ...
            console.error('[DualReportService] zstd 解压失败:', e)
          }
        }
      }
      // 尝试直接 UTF-8 解码
  ...
  /**
   * 统计指定会话的字数（文本消息内容的字符数）
   * 不再依赖 DLL 的 counts.words，因为 DLL 计算有严重偏差
   * 使用与 ChatPage 相同的逻辑：只统计 localType === 1 的纯文本消息
   */
  private async countTotalWords(
    ...
    const cursorResult = await wcdbService.openMessageCursor(sessionId, 1000, true, beginTimestamp, endTimestamp)
    if (!cursorResult.success || !cursorResult.cursor) return 0
    ...
    try {
      ...
      while (hasMore) {
        ...
        for (const row of batch.rows) {
          const localType = this.getRowInt(row, ['local_type', 'localType', 'type', 'msg_type', 'msgType', 'WCDB_CT_local_type'], 0)
          // 只统计纯文本消息（localType === 1），与 ChatPage 显示逻辑一致
          if (localType !== 1) continue

          const rawContent = this.decodeRowMessageContent(row)
          if (!rawContent || rawContent.length === 0) continue

          // 使用与 ChatPage 相同的逻辑处理内容
          // 1. 去除发送者前缀 (wxid_xxx: )
          // 2. 解码 HTML 实体
          // 3. 清理 UTF-16 代理对
          const cleanedContent = this.stripSenderPrefix(
            this.decodeHtmlEntities(this.cleanUtf16(rawContent))
          )

          // 只统计真正的文本内容长度
          if (cleanedContent && cleanedContent.length > 0) {
            totalWords += cleanedContent.length
          }
        }
        hasMore = batch.hasMore === true
      }
  ...
  /**
   * 去除发送者前缀，与 ChatService.stripSenderPrefix 逻辑一致
   */
  ...
  /**
   * 解码 HTML 实体，与 ChatService.decodeHtmlEntities 逻辑一致
   */
  ...
  /**
   * 清理 UTF-16 代理对，与 ChatService.cleanUtf16 逻辑一致
   */
  private cleanUtf16(content: string): string {
    if (!content) return ''
    // 移除孤立的代理对字符
  ...
  async generateReportWithConfig(params: {
    ...
    try {
      ...
      // 使用自己计算的字数，不再信任 DLL 的 counts.words
      this.reportProgress('统计字数...', 32, onProgress)
      const totalWords = await this.countTotalWords(friendUsername, startTime, endTime)

      const stats: DualReportStats = {
        totalMessages: counts.total || 0,
        totalWords: totalWords, // 使用自己计算的字数
