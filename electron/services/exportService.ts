...
// 导出诊断日志函数 - 写入应用数据目录确保可靠
function writeExportDiagLog(message: string): void {
  try {
    const logDir = path.join(app.getPath('userData'), 'logs')
    if (!fs.existsSync(logDir)) {
      fs.mkdirSync(logDir, { recursive: true })
    }
    const logPath = path.join(logDir, 'export-diag.log')
    const timestamp = new Date().toISOString()
    const line = `[${timestamp}] ${message}\n`
    fs.appendFileSync(logPath, line)
  } catch (e) {
    // 日志写入失败时忽略
    console.error('写入导出诊断日志失败:', e)
  }
}
...
const FORWARD_INDENT_STRING = '  '
const FORWARD_DEPTH_LIMIT_WARNING = '[...嵌套层级过深，已截断...]'

export interface ExportOptions {
  format: 'chatlab' | 'chatlab-jsonl' | 'json' | 'arkme-json' | 'html' | 'txt' | 'excel'
...
class ExportService {
  ...
  private readonly STOP_ERROR_CODE = 'CHATFLOW_EXPORT_STOP_REQUESTED'
  ...
  private getTargetMediaLocalTypes(options: ExportOptions): Set<number> {
    ...
    if (mediaContentType === 'file') return new Set([49, 8589934592049])
   ...
   * 解析合并转发的聊天记录 (Type 19)
   */
  private parseChatHistory(content: string, currentDepth = 0): ForwardChatRecordItem[] | undefined {
    try {
      const normalized = this.normalizeAppMessageContent(content || '')
      ...
      if (currentDepth >= MAX_FORWARD_DEPTH) {
        console.warn(`ExportService: 转发消息解析达到最大深度 ${MAX_FORWARD_DEPTH}，停止递归`)
        return undefined
      }
      ...
      while ((recordItemMatch = recordItemRegex.exec(normalized)) !== null) {
        const parsedItems = this.parseForwardChatRecordContainer(recordItemMatch[1] || '', currentDepth)
      ...
      if (items.length === 0 && normalized.includes('<dataitem')) {
        const fallbackItems = this.parseForwardChatRecordContainer(normalized, currentDepth)
  ...
  private parseForwardChatRecordContainer(containerXml: string, currentDepth = 0): ForwardChatRecordItem[] {
    const source = containerXml || ''
    if (!source) return []

    const nextDepth = currentDepth + 1
    if (nextDepth > MAX_FORWARD_DEPTH) {
      return []
    }
    ...
    for (const segment of segments) {
      ...
      while ((dataItemMatch = dataItemRegex.exec(segment)) !== null) {
        const parsed = this.parseForwardChatRecordDataItem(dataItemMatch[2] || '', dataItemMatch[1] || '', nextDepth)
        if (!parsed) continue
    ...
    const fallback = this.parseForwardChatRecordDataItem(source, '', nextDepth)
    return fallback ? [fallback] : []
  }

  private parseForwardChatRecordDataItem(body: string, attrs: string, currentDepth = 1): ForwardChatRecordItem | null {
    ...
    let nestedRecordList: ForwardChatRecordItem[] | undefined
    if (nestedRecordXml && currentDepth < MAX_FORWARD_DEPTH) {
      if (datatype === 17 || nestedRecordXml.includes('<recorditem') || nestedRecordXml.includes('<dataitem')) {
        nestedRecordList = this.parseForwardChatRecordContainer(nestedRecordXml, currentDepth)
        if (nestedRecordList && nestedRecordList.length === 0) {
          nestedRecordList = undefined
        }
      }
    } else if (nestedRecordXml && currentDepth >= MAX_FORWARD_DEPTH) {
      nestedRecordList = undefined
    }
  ...
  private buildForwardChatRecordLines(record: ForwardChatRecordItem, depth = 0): string[] {
    const safeDepth = Math.min(Math.max(depth, 0), MAX_FORWARD_DEPTH)
    const indent = safeDepth > 0 ? FORWARD_INDENT_STRING.repeat(safeDepth) : ''
    ...
    if (record.chatRecordList && record.chatRecordList.length > 0) {
      ...
      const depthIndicator = safeDepth > 0 ? ` [第${safeDepth}层转发]` : ''
      const header = `${indent}┌─ ${senderPrefix}[转发的聊天记录]${nestedTitle}${depthIndicator}`
      const nestedLines = record.chatRecordList.flatMap((item) => this.buildForwardChatRecordLines(item, safeDepth + 1))
      const footer = `${indent}└─ [结束第${safeDepth + 1}层嵌套]`
      return [header, ...nestedLines, footer]
    }
    
    const text = this.formatForwardChatRecordItemText(record)
    const prefix = safeDepth > 0 ? '│ ' : ''
    return [`${indent}${prefix}${senderPrefix}${text}`]
  }
  ...
  private applyExcelLinkCardCell(cell: any, content: string, localType: number): boolean {
    const linkCard = this.extractHtmlLinkCard(content, localType)
  ...
  private async collectMessages(
    ...
    console.log(`[Export] collectMessages开始: sessionId=${sessionId}, collectMode=${collectMode}, dateRange=${dateRange ? `${dateRange.start}-${dateRange.end}` : 'null'}`)
    ...
    if (!cursor.success || !cursor.cursor) {
      const errorMsg = `[Export] 打开游标失败: sessionId=${sessionId}, error=${cursor.error || '未知错误'}`
    ...
    console.log(`[Export] collectMessages完成: sessionId=${sessionId}, 收集到${rows.length}条消息, ${memberSet.size}个成员`)
  ...
  private getChatflowHeader(): { version: string; exportedAt: number; generator: string } {
    return {
      version: '1.0.3',
  ...
  async exportSessionToChatLab(
    ...
    try {
      this.throwIfStopRequested(control)
      writeExportDiagLog('检查数据库连接...')
      const conn = await this.ensureConnected()
      if (!conn.success || !conn.cleanedWxid) {
        writeExportDiagLog(`ERROR: 数据库连接失败: ${conn.error}`)
        return { success: false, error: conn.error }
      }
      writeExportDiagLog(`数据库连接成功, cleanedWxid: ${conn.cleanedWxid}`)

      const cleanedMyWxid = conn.cleanedWxid
      const isGroup = sessionId.includes('@chatroom')
      const rawMyWxid = String(this.configService.get('myWxid') || '').trim()
      writeExportDiagLog(`isGroup: ${isGroup}, rawMyWxid: ${rawMyWxid}`)

      writeExportDiagLog('获取会话信息...')
      const sessionInfo = await this.getContactInfo(sessionId)
      writeExportDiagLog(`sessionInfo: ${JSON.stringify(sessionInfo)}`)
      
      writeExportDiagLog('获取我的信息...')
      const myInfo = await this.getContactInfo(cleanedMyWxid)
      writeExportDiagLog(`myInfo获取成功`)
      ...
      if (!await this.pathExists(outputPath)) {
        return { success: false, error: `文件写入失败: ${outputPath}` }
      }
      const fileStats = await fs.promises.stat(outputPath)
      if (fileStats.size === 0) {
        return { success: false, error: `文件写入失败，文件大小为0字节: ${outputPath}` }
      }
  ...
  async exportSessionToDetailedJson(
    ...
    try {
      ...
      writeExportDiagLog(`消息收集完成, 共 ${totalMessages} 条消息`)

      // 如果没有消息，返回错误
      if (totalMessages === 0) {
        writeExportDiagLog('ERROR: 该会话没有消息')
        return { success: false, error: '该会话没有消息' }
      }
      ...
      const chatflow = this.getChatflowHeader()
  ...
  async exportSessionToExcel(
    ...
    writeExportDiagLog(`========== 开始导出Excel ==========`)
    writeExportDiagLog(`sessionId: ${sessionId}`)
    writeExportDiagLog(`outputPath: ${outputPath}`)
    writeExportDiagLog(`options: ${JSON.stringify(options)}`)
    
    try {
      this.throwIfStopRequested(control)

      // 测试文件系统是否工作
      writeExportDiagLog('开始文件系统测试...')
      try {
        const testDir = path.dirname(outputPath)
        writeExportDiagLog(`测试目录: ${testDir}`)
        
        await fs.promises.mkdir(testDir, { recursive: true })
        writeExportDiagLog('目录创建成功')
        
        const testFile = path.join(testDir, 'test-write.txt')
        await fs.promises.writeFile(testFile, 'test', 'utf-8')
        writeExportDiagLog('测试文件写入成功')
        
        const testContent = await fs.promises.readFile(testFile, 'utf-8')
        await fs.promises.unlink(testFile)
        writeExportDiagLog('测试文件读取并删除成功')
        
        if (testContent !== 'test') {
          const error = '文件系统测试失败：写入和读取内容不匹配'
          writeExportDiagLog(`ERROR: ${error}`)
          return { success: false, error }
        }
        writeExportDiagLog('文件系统测试通过')
      } catch (fsError) {
        const error = `文件系统测试失败: ${String(fsError)}`
        writeExportDiagLog(`ERROR: ${error}`)
        return { success: false, error }
      }
      ...
      // 确保目录存在
      const outputDir = path.dirname(outputPath)
      writeExportDiagLog(`确保输出目录存在: ${outputDir}`)
      await fs.promises.mkdir(outputDir, { recursive: true })
      writeExportDiagLog('输出目录创建/确认成功')

      // 写入文件
      this.throwIfStopRequested(control)
      writeExportDiagLog(`开始写入Excel文件: ${outputPath}`)
      try {
        await workbook.xlsx.writeFile(outputPath)
        writeExportDiagLog('Excel文件写入完成')
      } catch (writeError) {
        // 记录详细的写入错误
        const errorMsg = `Excel写入失败: ${String(writeError)}`
        writeExportDiagLog(`ERROR: ${errorMsg}`)
        const logPath = path.join(outputDir, 'export-error.log')
        const errorInfo = `Excel写入失败:\n时间: ${new Date().toISOString()}\n路径: ${outputPath}\n错误: ${String(writeError)}\n堆栈: ${writeError instanceof Error ? writeError.stack : '无'}\n`
        try {
      ...
      // 验证文件是否确实被写入且大小大于0
      writeExportDiagLog('验证文件是否存在...')
      if (!fs.existsSync(outputPath)) {
        const errorMsg = `文件写入失败: ${outputPath}`
        writeExportDiagLog(`ERROR: ${errorMsg}`)
        const logPath = path.join(outputDir, 'export-error.log')
        const errorInfo = `文件不存在:\n时间: ${new Date().toISOString()}\n路径: ${outputPath}\n`
        try {
          fs.appendFileSync(logPath, errorInfo)
        } catch {}
        return { success: false, error: errorMsg }
      }
      writeExportDiagLog('文件存在，检查文件大小...')
      const fileStats = fs.statSync(outputPath)
      writeExportDiagLog(`文件大小: ${fileStats.size} 字节`)
      if (fileStats.size === 0) {
        const errorMsg = `文件写入失败，文件大小为0字节: ${outputPath}`
        writeExportDiagLog(`ERROR: ${errorMsg}`)
        const logPath = path.join(outputDir, 'export-error.log')
        const errorInfo = `文件大小为0:\n时间: ${new Date().toISOString()}\n路径: ${outputPath}\n`
        try {
          fs.appendFileSync(logPath, errorInfo)
        } catch {}
        return { success: false, error: `文件写入失败，文件大小为0字节: ${outputPath}` }
      ...
      writeExportDiagLog(`========== 导出成功完成 ==========`)
      return { success: true }
    } catch (e) {
      const errorMsg = String(e)
      writeExportDiagLog(`ERROR: 导出异常: ${errorMsg}`)
      if (this.isStopError(e)) {
        return { success: false, error: '导出任务已停止' }
      }
      // 处理文件被占用的错误
  ...
  private async exportSessionToExcelStreaming(params: {
    ...
    try {
      ...
      appendRow(['导出工具', 'ChatFlow', '导出时间', this.formatTimestamp(Math.floor(Date.now() / 1000))])
      appendRow([])
...
`);
    ...
    <script>
      ...
      // Initial Data
      let allData = window.CHATFLOW_DATA || [];
      ...
      return new Promise((resolve, reject) => {
        ...
        stream.end(async () => {
          // 验证文件是否确实被写入且大小大于0
          if (!fs.existsSync(outputPath)) {
            reject(new Error(`文件写入失败: ${outputPath}`))
            return
          }
          const fileStats = fs.statSync(outputPath)
          if (fileStats.size === 0) {
            reject(new Error(`文件写入失败，文件大小为0字节: ${outputPath}`))
            return
          }
    ...
    try {
      ...
      if (cachedStatsEntry?.sessions) {
        for (const sessionId of sessionIds) {
          ...
          const totalCount = Number(snapshot.totalCount)
          if (Number.isFinite(totalCount) && totalCount >= 0) {
            sessionMessageCountHints.set(sessionId, Math.max(0, Math.floor(totalCount)))
          }
          ...
          for (let i = 0; i < precheckSessionIds.length; i += PRECHECK_BATCH_SIZE) {
            ...
            if (countsResult.success && countsResult.counts) {
              for (const batchSessionId of batchSessionIds) {
                ...
                if (count === undefined) {
                  console.log(`[Export] 预检查: 会话 ${batchSessionId} 消息数量查询失败，将尝试正常导出`)
                }
              }
            }
      ...
      if (emptySessionIds.size > 0) {
        console.log(`[Export] 预检查完成: 识别到 ${emptySessionIds.size} 个空会话将跳过:`, Array.from(emptySessionIds).slice(0, 10))
      }
      ...
      const runOne = async (sessionId: string): Promise<'done' | 'stopped'> => {
        try {
          ...
          // 注释掉空会话跳过逻辑，确保即使会话为空也会尝试导出（生成带头部信息的空文件）
          // if (
          ...
          //   messageCountHint === 0
          // ) {
          //   console.log(`[Export] 跳过空会话: ${sessionId} (确认消息数量为0)`)
          //   failCount++
          ...
          //   console.log(`[Export] 跳过空会话(预检查): ${sessionId}`)
          //   failCount++
          //   failedSessionIds.push(sessionId)
          //   activeSessionRatios.delete(sessionId)
          //   completedCount++
      ...
      if (successCount === 0 && failCount > 0) {
        return { success: false, successCount, failCount, error: '所有会话导出失败', successSessionIds, failedSessionIds }
      }
