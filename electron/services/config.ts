...
interface ConfigSchema {
  ...
  // 防撤回通知
  antiRevokeNotificationEnabled: boolean
  antiRevokeNotificationShowContent: boolean
}
...
export class ConfigService {
  ...
  constructor() {
    ...
    const defaults: ConfigSchema = {
      ...
      antiRevokeNotificationEnabled: false,
      antiRevokeNotificationShowContent: true
    }

    const storeOptions: any = {
      name: 'ChatFlow-config',
      defaults,
      projectName: String(process.env.CHATFLOW_PROJECT_NAME || 'ChatFlow').trim() || 'ChatFlow'
    }
    const runningInWorker = process.env.CHATFLOW_WORKER === '1'
    if (runningInWorker) {
      const cwd = String(process.env.CHATFLOW_CONFIG_CWD || process.env.CHATFLOW_USER_DATA_PATH || '').trim()
      if (cwd) {
    ...
    try {
      ...
      if (message.includes('projectName')) {
        const fallbackOptions = {
          ...
          cwd: storeOptions.cwd || process.env.CHATFLOW_CONFIG_CWD || process.env.CHATFLOW_USER_DATA_PATH || process.cwd()
        }
  ...
  private getUserDataPath(): string {
    const workerUserDataPath = String(process.env.CHATFLOW_USER_DATA_PATH || process.env.CHATFLOW_CONFIG_CWD || '').trim()
