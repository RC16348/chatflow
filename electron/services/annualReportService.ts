...
class AnnualReportService {
  ...
  async generateReportWithConfig(params: {
    ...
    try {
      ...
      // 消息类型统计
      const messageTypeCounts = { text: 0, image: 0, voice: 0, video: 0, emoji: 0, link: 0, other: 0 }
      // 月度趋势统计
      const monthlyTrendMap = new Map<number, { month: number; sent: number; received: number }>()
      // 发送/接收总计
      let totalSent = 0
      let totalReceived = 0
      ...
      // 从原生层结果汇总发送/接收总数
      for (const stat of contactStats.values()) {
        totalSent += stat.sent
      ...
      // 无论是否使用 SQL 扩展统计，都需要获取消息类型统计
      this.reportProgress('统计消息类型分布...', 42, onProgress)
      try {
        // 使用 getAggregateStats 获取 typeCounts 格式数据（更可靠）
        const aggregateResult = await wcdbService.getAggregateStats(sessionIds, actualStartTime, actualEndTime)
        if (aggregateResult.success && aggregateResult.data && aggregateResult.data.typeCounts) {
          const typeCounts = aggregateResult.data.typeCounts as Record<string, number>
          // typeCounts 格式: { "1": 100, "3": 50, "49": 20, ... }
          messageTypeCounts.text += Math.max(0, Math.floor(Number(typeCounts['1'] || 0)))
          messageTypeCounts.image += Math.max(0, Math.floor(Number(typeCounts['3'] || 0)))
          messageTypeCounts.voice += Math.max(0, Math.floor(Number(typeCounts['34'] || 0)))
          messageTypeCounts.video += Math.max(0, Math.floor(Number(typeCounts['43'] || 0)))
          messageTypeCounts.emoji += Math.max(0, Math.floor(Number(typeCounts['47'] || 0)))
          messageTypeCounts.link += Math.max(0, Math.floor(Number(typeCounts['49'] || 0)))
          // 其他类型（通话、转账、红包等）归入 other
          const otherTypes = ['50', '8589934592049', '8594229559345'] // 通话、转账、红包
          let otherSum = 0
          for (const type of otherTypes) {
            otherSum += Math.max(0, Math.floor(Number(typeCounts[type] || 0)))
          }
          // 将所有未明确分类的类型也归入 other
          for (const [typeStr, count] of Object.entries(typeCounts)) {
            if (!['1', '3', '34', '43', '47', '49'].includes(typeStr)) {
              otherSum += Math.max(0, Math.floor(Number(count || 0)))
            }
          }
          messageTypeCounts.other += otherSum
        }
      } catch {
        // 如果批量获取失败，后续遍历时会统计
      }
      ...
      // 判断是否需要遍历消息来补充统计
      // 月度趋势需要区分发送/接收，原生层未返回此细分数据
      // 消息类型统计如果批量接口返回的数据不完整（某些类型为0但总数不为0），也需要通过遍历补充
      const needMonthlyTrend = monthlyTrendMap.size === 0
      const totalTypedMessages = messageTypeCounts.text + messageTypeCounts.image + messageTypeCounts.voice + messageTypeCounts.video + messageTypeCounts.emoji + messageTypeCounts.link + messageTypeCounts.other
      // 如果批量接口没有返回任何数据，或者某些类型为0但总数不为0（说明批量接口数据不完整），都需要遍历统计
      // 特别检查链接(type=49)，因为批量接口经常返回0
      const hasIncompleteData = totalTypedMessages > 0 && (messageTypeCounts.text === 0 || messageTypeCounts.link === 0)
      const needMessageTypeStats = totalTypedMessages === 0 || hasIncompleteData
      // 如果批量接口数据不完整，需要清空后重新统计，避免重复累加
      if (hasIncompleteData) {
        messageTypeCounts.text = 0
        messageTypeCounts.image = 0
        messageTypeCounts.voice = 0
        messageTypeCounts.video = 0
        messageTypeCounts.emoji = 0
        messageTypeCounts.link = 0
        messageTypeCounts.other = 0
      }
      const needFullAnalysis = !useSqlExtras

      if (needFullAnalysis || needMonthlyTrend || needMessageTypeStats) {
          ...
          try {
            ...
            while (hasMore) {
              ...
              for (const row of batch.rows) {
                ...
                let isSent: boolean
                if (typeof isSendRaw === 'boolean') {
                  isSent = isSendRaw
                } else {
                  isSent = parseInt(isSendRaw, 10) === 1
                }
                const localType = parseInt(row.local_type || row.localType || row.WCDB_CT_local_type || row.type || '1', 10)

                // 兼容逻辑：当 is_send 字段缺失或无效时，通过 sender 判断
                // 注意：'0' 是有效的值（表示接收），只有 undefined/null/空字符串才需要兼容判断
                const needsFallback = isSendRaw === undefined || isSendRaw === null || isSendRaw === ''
                if (needsFallback) {
                  ...
                  if (sender) {
                    ...
                    if (sender === rawLower || sender === cleanedLower ||
                        sender.startsWith(rawLower + '_') || sender.startsWith(cleanedLower + '_') ||
                ...
                // 消息类型统计（仅在需要时统计）
                if (needMessageTypeStats) {
                  if (localType === 1 || localType === 244813135921) messageTypeCounts.text++
                  else if (localType === 3) messageTypeCounts.image++
                  else if (localType === 34) messageTypeCounts.voice++
                  else if (localType === 43) messageTypeCounts.video++
                  else if (localType === 47) messageTypeCounts.emoji++
                  else if (localType === 49) messageTypeCounts.link++
                  else messageTypeCounts.other++
                }

                // 月度趋势统计（仅在需要时统计）
                const dt = new Date(createTime * 1000)
                if (needMonthlyTrend) {
                  const monthKey = dt.getFullYear() * 100 + (dt.getMonth() + 1)
                  if (!monthlyTrendMap.has(monthKey)) {
                    monthlyTrendMap.set(monthKey, { month: dt.getMonth() + 1, sent: 0, received: 0 })
                  }
                  const monthStats = monthlyTrendMap.get(monthKey)!
                  if (isSent) monthStats.sent++
                  else monthStats.received++
                }

                // 常用语（仅在需要完整分析时统计）
                if (needFullAnalysis && (localType === 1 || localType === 244813135921) && isSent) {
                ...
                // 深夜统计（仅在需要完整分析时统计）
                if (needFullAnalysis && dt.getHours() >= 0 && dt.getHours() < 6) {
                  midnightStats.set(sessionId, (midnightStats.get(sessionId) || 0) + 1)
                }

                // 峰值日联系人统计（仅在需要完整分析时统计）
                if (needFullAnalysis && peakDayKey) {
      ...
      let totalConvReceived = 0
      for (const stats of conversationStarts.values()) {
        totalInitiated += stats.initiated
        totalConvReceived += stats.received
      }
      const totalConversations = totalInitiated + totalConvReceived
      if (totalConversations > 0) {
        socialInitiative = {
          initiatedChats: totalInitiated,
          receivedChats: totalConvReceived,
      ...
      const reportData: AnnualReportData = {
        ...
        messageTypeStats: messageTypeCounts,
        monthlyTrend: Array.from(monthlyTrendMap.values()).sort((a, b) => a.month - b.month),
        coreFriends,
        monthlyTopFriends,
        peakDay,
        longestStreak,
