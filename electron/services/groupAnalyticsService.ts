...
class GroupAnalyticsService {
    ...
    try {
      ...
      for (const rank of rankings) {
        ...
        const possibleKeys = [
          ...
          rank.member.username?.replace(/^wxid_/, ''),
          rank.member.username?.includes('@') ? rank.member.username.split('@')[0] : rank.member.username
        ].filter(Boolean)

        let foundDisplayName = false
        for (const key of possibleKeys) {
          if (names.success && names.map && names.map[key] && names.map[key] !== key) {
            rank.member.displayName = names.map[key]
            foundDisplayName = true
            break
          }
        }

        // 如果没找到displayName，尝试从原始senderEntries中找回对应的原始ID
        if (!foundDisplayName) {
          const originalEntry = senderEntries.find(([id, count]) => {
            const mappedUsername = idMap[id] || id
            return mappedUsername === rank.member.username
          })
          if (originalEntry) {
            const originalId = originalEntry[0]
            if (names.success && names.map && names.map[originalId] && names.map[originalId] !== originalId) {
              rank.member.displayName = names.map[originalId]
              foundDisplayName = true
            }
          }
        }

        if (avatars.success && avatars.map) {
          for (const key of possibleKeys) {
            if (avatars.map[key]) {
              rank.member.avatarUrl = avatars.map[key]
              break
            }
          }
        }
  ...
  async exportGroupMemberMessages(
    ...
    try {
      ...
      const exportGenerator = 'ChatFlow'
