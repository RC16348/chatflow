...
import { simpleInfo, simpleDebug, simpleError, getSimpleLogPath } from './simpleDebugLog'
import { antiRevokeNotificationService } from './antiRevokeNotificationService'
...
class ChatService {
  ...
  async connect(): Promise<{ success: boolean; error?: string }> {
    console.log('[ChatService] connect() 开始')
    try {
      if (this.connected && wcdbService.isReady()) {
        console.log('[ChatService] 已经连接且 WCDB 就绪')
        return { success: true }
      }
      console.log('[ChatService] 检查配置...')
      const wxid = this.configService.get('myWxid')
      const dbPath = this.configService.get('dbPath')
      const decryptKey = this.configService.get('decryptKey')
      console.log(`[ChatService] wxid=${wxid ? '已设置' : '未设置'}, dbPath=${dbPath ? '已设置' : '未设置'}, decryptKey=${decryptKey ? '已设置' : '未设置'}`)
      ...
      console.log(`[ChatService] 调用 wcdbService.open()... cleanedWxid=${cleanedWxid}`)
      const openOk = await wcdbService.open(dbPath, decryptKey, cleanedWxid)
      console.log(`[ChatService] wcdbService.open() 返回: ${openOk}`)
      if (!openOk) {
        const lastError = await wcdbService.getLastInitError()
        console.log(`[ChatService] 打开失败，错误: ${lastError}`)
        const detailedError = this.toCodeOnlyMessage(lastError)
  ...
  private setupDbMonitor() {
    // 不再使用 monitorSetup 标志阻止重试
    // 每次 connect 成功都尝试设置监控（wcdbService 内部会检查 monitorSetup 避免重复启动）
  ...
  async getMessages(
    ...
    try {
      ...
      if (needNewCursor) {
        ...
        // 创建新游标
        // 注意：ChatFlow 数据库中的 create_time 是以秒为单位的
      ...
      // 检查消息中是否包含撤回消息，如果是则发送通知
      antiRevokeNotificationService.checkMessagesForRevoke(sessionId, filtered)
  ...
  /**
   * 从本地 ChatFlow emoji 缓存目录按 md5 查找文件
   */
    ...
    try {
      ...
      try {
        ...
        if (collected.messages) {
          antiRevokeNotificationService.checkMessagesForRevoke(sessionId, collected.messages)
        }
  ...
  async getNewMessages(sessionId: string, minTime: number, limit: number = this.messageBatchDefault): Promise<{ success: boolean; messages?: Message[]; error?: string }> {
    try {
      ...
      // 检查消息中是否包含撤回消息，如果是则发送通知
      antiRevokeNotificationService.checkMessagesForRevoke(sessionId, normalized)
  ...
  private async hydrateContactsWithExtendedFields(rows: Record<string, any>[]): Promise<Record<string, any>[]> {
    ...
    try {
      ...
      if (!selectableColumns) {
        ...
        // 输出contact表结构到日志文件，用于分析性别字段
        try {
          const logDir = join(app.getPath('userData'), 'logs')
          if (!existsSync(logDir)) {
            mkdirSync(logDir, { recursive: true })
          }
          const logPath = join(logDir, 'contact_table_structure.log')
          const timestamp = new Date().toISOString()
          let logContent = `\n========================================\n`
          logContent += `Contact Table Structure - ${timestamp}\n`
          logContent += `========================================\n\n`
          logContent += `Total Fields: ${tableInfoResult.rows.length}\n\n`
          logContent += `Field List (Name | Type | NotNull | DefaultValue | PrimaryKey):\n`
          logContent += `-`.repeat(80) + `\n`

          // 输出所有字段的完整信息
          for (const tableInfoRow of tableInfoResult.rows as Record<string, any>[]) {
            const cid = tableInfoRow.cid ?? tableInfoRow.cid ?? ''
            const rawName = tableInfoRow.name ?? tableInfoRow.column_name ?? tableInfoRow.columnName
            const name = String(rawName || '').trim()
            const type = tableInfoRow.type ?? 'UNKNOWN'
            const notNull = tableInfoRow.notnull ?? tableInfoRow.notNull ?? 0
            const dfltValue = tableInfoRow.dflt_value ?? tableInfoRow.defaultValue ?? ''
            const pk = tableInfoRow.pk ?? tableInfoRow.primaryKey ?? 0

            if (!name) continue

            logContent += `  ${cid.toString().padStart(3)} | ${name.padEnd(25)} | ${type.padEnd(15)} | ${notNull.toString().padStart(7)} | ${String(dfltValue).padEnd(15)} | ${pk}\n`
          }

          logContent += `\n========================================\n\n`

          // 查询多条记录的extra_buffer字段进行解析，用于分析性别字段
          try {
            const sampleResult = await wcdbService.execQuery('contact', null, `SELECT username, nick_name, extra_buffer FROM contact WHERE extra_buffer IS NOT NULL AND nick_name IS NOT NULL AND username NOT LIKE '%@chatroom%' LIMIT 200`)
            if (sampleResult.success && Array.isArray(sampleResult.rows) && sampleResult.rows.length > 0) {
              logContent += `Sample extra_buffer parsing (first 200 contacts):\n`
              logContent += `-`.repeat(80) + `\n`

              // 收集所有varint字段用于分析
              const varintFieldsAnalysis: Record<number, number[]> = {}
              let successCount = 0
              let errorCount = 0

              for (let i = 0; i < (sampleResult.rows as Record<string, any>[]).length; i++) {
                const row = (sampleResult.rows as Record<string, any>[])[i]
                // 为每个联系人单独处理，避免单个错误影响整体
                try {
                  const username = row.username || 'N/A'
                  const nickName = row.nick_name || 'N/A'
                  logContent += `\n[${i + 1}] Contact: ${nickName} (${username})\n`

                  // 尝试解析extra_buffer
                  const extraBuffer = this.toExtraBufferBytes(row)
                  if (extraBuffer) {
                    logContent += `  extra_buffer length: ${extraBuffer.length} bytes\n`

                    // 解析所有字段
                    logContent += `  Parsed fields:\n`
                    const fields = this.parseAllExtraBufferFields(row)
                    for (const [fieldNum, values] of Object.entries(fields)) {
                      for (const value of values) {
                        logContent += `    Field ${fieldNum}: ${value.substring(0, 100)}${value.length > 100 ? '...' : ''}\n`

                        // 收集varint值用于分析
                        const varintMatch = value.match(/\[varint:(\d+)\]/)
                        if (varintMatch) {
                          const fieldNumInt = parseInt(fieldNum)
                          const val = parseInt(varintMatch[1])
                          if (!varintFieldsAnalysis[fieldNumInt]) {
                            varintFieldsAnalysis[fieldNumInt] = []
                          }
                          varintFieldsAnalysis[fieldNumInt].push(val)
                        }
                      }
                    }
                  } else {
                    logContent += `  extra_buffer: null or empty\n`
                  }
                  successCount++
                } catch (contactError) {
                  errorCount++
                  logContent += `  [ERROR] Failed to parse contact ${i + 1}: ${contactError}\n`
                }
              }

              logContent += `\n\nParsing Summary: ${successCount} succeeded, ${errorCount} failed\n`

              // 分析varint字段的分布，找出可能是性别的字段
              logContent += `\n\nVarint Fields Analysis (possible gender field candidates):\n`
              logContent += `-`.repeat(80) + `\n`
              for (const [fieldNum, values] of Object.entries(varintFieldsAnalysis)) {
                const uniqueValues = [...new Set(values)].sort((a, b) => a - b)
                const valueCounts: Record<number, number> = {}
                values.forEach(v => { valueCounts[v] = (valueCounts[v] || 0) + 1 })

                // 如果字段只有0,1,2三个值，可能是性别字段
                const isPossibleGender = uniqueValues.length <= 3 && uniqueValues.every(v => v >= 0 && v <= 2)
                const marker = isPossibleGender ? ' <-- [POSSIBLE GENDER FIELD]' : ''

                logContent += `  Field ${fieldNum}:${marker}\n`
                logContent += `    Unique values: ${uniqueValues.join(', ')}\n`
                logContent += `    Distribution: ${Object.entries(valueCounts).map(([k, v]) => `${k}=${v}`).join(', ')}\n`
              }

              logContent += `\n========================================\n\n`
            }
          } catch (sampleError) {
            logContent += `\nFailed to parse extra_buffer samples: ${sampleError}\n\n`
          }

          writeFileSync(logPath, logContent, { flag: 'a' })
          console.log(`[Contact Table] 表结构已输出到: ${logPath}`)
        } catch (logError) {
          console.warn('[Contact Table] 输出表结构日志失败:', logError)
        }
  ...
  /**
   * 从extra_buffer中解析性别字段
   * 性别存储在Field 2，取值：0=未知，1=男，2=女
   */
  private parseGenderFromExtraBuffer(row: Record<string, any>): number {
    try {
      ...
      console.warn('[parseGenderFromExtraBuffer] 解析失败:', e)
    }
    return 0
  }
  ...
  /**
   * 解析extra_buffer中的所有字段
   * 返回一个对象，key是字段编号，value是该字段的所有字符串值数组
   */
  private parseAllExtraBufferFields(row: Record<string, any>): Record<number, string[]> {
    ...
    try {
      ...
      while (offset < bytes.length) {
        ...
        if (wireType === 0) {
          ...
          if (!varint) break
          offset = varint.nextOffset
          // 记录varint值（可能是性别等数字字段）
          if (!result[fieldNumber]) result[fieldNumber] = []
          result[fieldNumber].push(`[varint:${varint.value}]`)
          continue
        }
        ...
        if (wireType === 1) {
          ...
          offset += 8
          if (!result[fieldNumber]) result[fieldNumber] = []
          result[fieldNumber].push(`[double:${value}]`)
          continue
        }
        ...
        if (wireType === 2) {
          ...
          if (text && text.length > 0) {
            result[fieldNumber].push(text)
          } else {
            // 如果不是有效字符串，记录hex
            result[fieldNumber].push(`[hex:${payload.toString('hex').substring(0, 50)}]`)
          }
          continue
        }
        ...
        if (wireType === 5) {
          ...
          offset += 4
          if (!result[fieldNumber]) result[fieldNumber] = []
          result[fieldNumber].push(`[float:${value}]`)
          continue
        }
      ...
      // 捕获所有错误，避免影响主程序
      console.warn('[parseAllExtraBufferFields] 解析失败:', e)
    }
    return result
  }
  ...
  private async getFriendIdentitySet(): Promise<Set<string>> {
    ...
    simpleInfo('[getFriendIdentitySet] ========== 开始构建好友身份集 ==========')
    
    const contactResult = await wcdbService.getContactsCompact()
    if (!contactResult.success || !contactResult.contacts) {
      simpleError(`[getFriendIdentitySet] 获取联系人失败: ${contactResult.error}`)
      return identities
    }
    simpleInfo(`[getFriendIdentitySet] 获取到 ${contactResult.contacts.length} 个联系人`)

    // 收集所有可能的用户名用于批量查询好友标记
    const allUsernames: string[] = []

    for (const rowAny of contactResult.contacts) {
      const row = rowAny as Record<string, any>

      const username = String(this.getRowField(row, ['username', 'WCDB_CT_username']) || '').trim()
      const userName = String(this.getRowField(row, ['user_name', 'userName', 'WCDB_CT_user_name']) || '').trim()
      const encryptUsername = String(this.getRowField(row, ['encrypt_username', 'encryptUsername', 'WCDB_CT_encrypt_username']) || '').trim()
      const encryptUserName = String(this.getRowField(row, ['encrypt_user_name', 'encryptUserName', 'WCDB_CT_encrypt_user_name']) || '').trim()
      const alias = String(this.getRowField(row, ['alias', 'WCDB_CT_alias']) || '').trim()

      // 收集所有可能的用户名
      const names = [username, userName, encryptUsername, encryptUserName, alias].filter(Boolean) as string[]
      for (const name of names) {
        if (!name.includes('@chatroom') && !name.startsWith('gh_') && !FRIEND_EXCLUDE_USERNAMES.has(name)) {
          allUsernames.push(name)
        }
      }
    }

    // 使用 getContactFriendFlags 批量查询好友标记（更可靠的方法）
    const uniqueUsernames = Array.from(new Set(allUsernames))
    simpleInfo(`[getFriendIdentitySet] 准备查询 ${uniqueUsernames.length} 个用户名的好友标记`)
    simpleDebug(`[getFriendIdentitySet] 待查询用户名列表: ${JSON.stringify(uniqueUsernames.slice(0, 20))}${uniqueUsernames.length > 20 ? '...' : ''}`)
    
    let friendFlagsMap: Record<string, boolean> = {}
    let flagsFromApiCount = 0
    if (uniqueUsernames.length > 0) {
      try {
        const flagsResult = await wcdbService.getContactFriendFlags(uniqueUsernames)
        simpleInfo(`[getFriendIdentitySet] API查询结果: success=${flagsResult.success}, map大小=${flagsResult.map ? Object.keys(flagsResult.map).length : 0}`)
        
        if (flagsResult.success && flagsResult.map) {
          friendFlagsMap = flagsResult.map
          flagsFromApiCount = Object.values(friendFlagsMap).filter(Boolean).length
          simpleInfo(`[getFriendIdentitySet] API查询成功: ${flagsFromApiCount} 个好友标记`)
          simpleDebug(`[getFriendIdentitySet] 好友标记详情: ${JSON.stringify(Object.entries(flagsResult.map).filter(([k, v]) => v).slice(0, 20))}`)
        } else {
          simpleError(`[getFriendIdentitySet] API查询失败: ${flagsResult.error}`)
        }
      } catch (e) {
        simpleError('[getFriendIdentitySet] 批量查询好友标记失败:', e)
      }
    }

    let fromApiCount = 0
    let fromLocalTypeCount = 0
    ...
    for (const rowAny of contactResult.contacts) {
      ...
      const localType = this.getRowInt(row, ['local_type', 'localType', 'WCDB_CT_local_type'], 0)
      const quanPinRaw = this.getRowField(row, ['quan_pin', 'quanPin', 'WCDB_CT_quan_pin'])
      const quanPin = String(quanPinRaw || '').trim()

      const username = String(this.getRowField(row, ['username', 'WCDB_CT_username']) || '').trim()

      // 判断是否为好友：优先使用 friendFlagsMap，其次使用 localType/quanPin
      let isValidContact = false
      let reason = ''
      if (username && friendFlagsMap[username] === true) {
        isValidContact = true
        fromApiCount++
        reason = 'API'
      } else if (localType === 1 || (localType === 0 && quanPin)) {
        isValidContact = true
        fromLocalTypeCount++
        reason = 'localType'
      }
      ...
      if (userName && userName !== username) usernames.push(userName)
      if (encryptUsername && encryptUsername !== username && encryptUsername !== userName) usernames.push(encryptUsername)
      if (encryptUserName && encryptUserName !== username && encryptUserName !== userName && encryptUserName !== encryptUsername) usernames.push(encryptUserName)
      if (alias && alias !== username && alias !== userName && alias !== encryptUsername && alias !== encryptUserName) usernames.push(alias)

      for (const name of usernames) {
        if (!name || name.includes('@chatroom') || name.startsWith('gh_')) continue
        if (FRIEND_EXCLUDE_USERNAMES.has(name)) continue
      ...
      if (processedCount <= 10) {
        simpleDebug(`[getFriendIdentitySet] 处理联系人 ${username}: reason=${reason}, usernames=${JSON.stringify(usernames)}, keys=${JSON.stringify(usernames.flatMap(n => this.buildIdentityKeys(n)))}`)
      }
    }
    
    simpleInfo('[getFriendIdentitySet] ========== 完成 ==========')
    simpleInfo(`[getFriendIdentitySet] 总共 ${identities.size} 个身份标识 (API判断:${fromApiCount}, localType判断:${fromLocalTypeCount})`)
    simpleDebug(`[getFriendIdentitySet] 身份标识详情: ${JSON.stringify(Array.from(identities).slice(0, 50))}${identities.size > 50 ? '...' : ''}`)
    return identities
  }
  ...
  private async buildGroupRelationStats(
    ...
    simpleInfo(`[buildGroupRelationStats] ========== 开始计算 ==========`)
    simpleInfo(`[buildGroupRelationStats] 群聊数量: ${groupSessionIds.length}, 私聊数量: ${privateSessionIds.length}`)
    simpleInfo(`[buildGroupRelationStats] 群聊列表: ${JSON.stringify(groupSessionIds)}`)
    simpleInfo(`[buildGroupRelationStats] 私聊列表: ${JSON.stringify(privateSessionIds)}`)
    simpleInfo(`[buildGroupRelationStats] 自己身份集: ${JSON.stringify(Array.from(selfIdentitySet))}`)
    ...
    // 构建私聊会话索引 - 使用多种键格式以提高匹配率
    const privateIndex = new Map<string, Set<string>>()
    for (const sessionId of privateSessionIds) {
      // 原始 sessionId
      const originalKeys = this.buildIdentityKeys(sessionId)
      simpleDebug(`[buildGroupRelationStats] 私聊 ${sessionId} 的原始keys: ${JSON.stringify(originalKeys)}`)
      ...
      if (cleanedSessionId !== sessionId) {
        const cleanedKeys = this.buildIdentityKeys(cleanedSessionId)
        simpleDebug(`[buildGroupRelationStats] 私聊 ${sessionId} 的清理后keys: ${JSON.stringify(cleanedKeys)}`)
    ...
    simpleDebug(`[buildGroupRelationStats] 私聊索引详情: ${JSON.stringify(Array.from(privateIndex.entries()).map(([k, v]) => [k, Array.from(v)]))}`)

    const friendIdentitySet = await this.getFriendIdentitySet()
    simpleInfo(`[buildGroupRelationStats] 好友身份集大小: ${friendIdentitySet.size}`)
    simpleDebug(`[buildGroupRelationStats] 好友身份集详情: ${JSON.stringify(Array.from(friendIdentitySet))}`)

    // 收集所有群成员用户名用于批量查询好友标记
    const allGroupMembers = new Set<string>()
    const groupMembersMap = new Map<string, string[]>() // groupId -> usernames[]

    for (const groupId of groupSessionIds) {
      const membersResult = await wcdbService.getGroupMembers(groupId)
      simpleInfo(`[buildGroupRelationStats] 群聊 ${groupId} 获取成员结果: success=${membersResult.success}, members数量=${membersResult.members?.length || 0}`)
      
      if (membersResult.success && membersResult.members) {
        const usernames: string[] = []
        for (const member of membersResult.members) {
          const username = this.extractGroupMemberUsername(member)
          simpleDebug(`[buildGroupRelationStats] 群聊 ${groupId} 成员原始数据: ${JSON.stringify(member)} -> 提取用户名: ${username}`)
          
          if (username) {
            usernames.push(username)
            allGroupMembers.add(username)
          }
        }
        groupMembersMap.set(groupId, usernames)
        simpleInfo(`[buildGroupRelationStats] 群聊 ${groupId}: ${usernames.length}个成员, 用户名列表: ${JSON.stringify(usernames)}`)
      } else {
        simpleError(`[buildGroupRelationStats] 群聊 ${groupId}: 获取成员失败: ${membersResult.error}`)
      }
    }
    simpleInfo(`[buildGroupRelationStats] 总共收集 ${allGroupMembers.size}个唯一群成员`)

    // 批量查询群成员的好友标记
    const groupMemberFlagsMap = new Map<string, boolean>()
    const uniqueGroupMembers = Array.from(allGroupMembers)
    simpleInfo(`[buildGroupRelationStats] 准备批量查询 ${uniqueGroupMembers.length}个成员的好友标记`)
    simpleDebug(`[buildGroupRelationStats] 待查询成员列表: ${JSON.stringify(uniqueGroupMembers)}`)
    
    if (uniqueGroupMembers.length > 0) {
      try {
        const flagsResult = await wcdbService.getContactFriendFlags(uniqueGroupMembers)
        simpleInfo(`[buildGroupRelationStats] 好友标记查询结果: success=${flagsResult.success}, map大小=${flagsResult.map ? Object.keys(flagsResult.map).length : 0}`)
        
        if (flagsResult.success && flagsResult.map) {
          let friendCount = 0
          for (const [username, isFriend] of Object.entries(flagsResult.map)) {
            groupMemberFlagsMap.set(username, Boolean(isFriend))
            if (isFriend) friendCount++
          }
          simpleInfo(`[buildGroupRelationStats] 好友标记查询成功: ${friendCount}/${Object.keys(flagsResult.map).length}个好友`)
          simpleDebug(`[buildGroupRelationStats] 好友标记详情: ${JSON.stringify(Object.entries(flagsResult.map).filter(([k, v]) => v))}`)
        } else {
          simpleError(`[buildGroupRelationStats] 好友标记查询失败: ${flagsResult.error}`)
        }
      } catch (e) {
        simpleError('[buildGroupRelationStats] 批量查询群成员好友标记失败:', e)
      }
    }

    await this.forEachWithConcurrency(groupSessionIds, 4, async (groupId) => {
      // 使用预先查询的群成员数据，避免重复查询
      const membersUsernames = groupMembersMap.get(groupId) || []
      if (membersUsernames.length === 0) {
      ...
      for (const username of membersUsernames) {
        if (!username) continue

        // 构建多种格式的 identity keys 以提高匹配率
        const identityKeys = this.buildIdentityKeys(username)
        const cleanedUsername = this.cleanAccountDirName(username)
        if (cleanedUsername !== username) {
          const cleanedKeys = this.buildIdentityKeys(cleanedUsername)
          for (const key of cleanedKeys) {
        ...
        simpleDebug(`[buildGroupRelationStats] 处理成员 ${username}: identityKeys=${JSON.stringify(identityKeys)}`)

        // 判断是否为好友：优先使用批量查询结果，其次使用 friendIdentitySet
        let isFriend = false
        let foundInFlags = false
        let matchedFlagKey = ''
        // 尝试多种格式的用户名查找好友标记
        for (const key of identityKeys) {
          if (groupMemberFlagsMap.has(key)) {
            isFriend = groupMemberFlagsMap.get(key)!
            foundInFlags = true
            matchedFlagKey = key
            break
          }
        }
        // 如果批量查询没有结果，尝试使用 friendIdentitySet
        if (!isFriend && !foundInFlags) {
          for (const key of identityKeys) {
        ...
        simpleDebug(`[buildGroupRelationStats] 成员 ${username}: isFriend=${isFriend}, foundInFlags=${foundInFlags}, matchedKey=${matchedFlagKey}`)

        const canonical = identityKeys[0]
        if (!selfIdentitySet.has(canonical) && isFriend) {
          friendMembers.add(canonical)
          simpleDebug(`[buildGroupRelationStats] 成员 ${username} 被计入共同好友`)
        }

        // 查找匹配的私聊会话（使用多种键格式）
        let matchedPrivateSession = false
        for (const key of identityKeys) {
          const linked = privateIndex.get(key)
          if (!linked) continue
          for (const sessionId of linked) {
            touchedPrivateSessions.add(sessionId)
            matchedPrivateSession = true
            simpleDebug(`[buildGroupRelationStats] 成员 ${username} 匹配到私聊 ${sessionId} (通过key: ${key})`)
          }
        }
        if (!matchedPrivateSession) {
          simpleDebug(`[buildGroupRelationStats] 成员 ${username} 未匹配到任何私聊`)
        }
      }
      ...
      simpleInfo(`[buildGroupRelationStats] 群聊 ${groupId}: 共同好友=${friendMembers.size}, 被触达私聊=${touchedPrivateSessions.size}`)
      simpleDebug(`[buildGroupRelationStats] 群聊 ${groupId} 共同好友列表: ${JSON.stringify(Array.from(friendMembers))}`)
      simpleDebug(`[buildGroupRelationStats] 群聊 ${groupId} 被触达私聊列表: ${JSON.stringify(Array.from(touchedPrivateSessions))}`)
    })

    // 输出最终结果统计
    const totalMutualGroups = Object.values(privateMutualGroupMap).reduce((a, b) => a + b, 0)
    const totalMutualFriends = Object.values(groupMutualFriendMap).reduce((a, b) => a + b, 0)
    simpleInfo(`[buildGroupRelationStats] ========== 完成 ==========`)
    simpleInfo(`[buildGroupRelationStats] 总共同群聊=${totalMutualGroups}, 总群共同好友=${totalMutualFriends}`)
    simpleInfo(`[buildGroupRelationStats] privateMutualGroupMap: ${JSON.stringify(privateMutualGroupMap)}`)
    simpleInfo(`[buildGroupRelationStats] groupMutualFriendMap: ${JSON.stringify(groupMutualFriendMap)}`)
    simpleInfo(`[buildGroupRelationStats] 日志文件位置: ${getSimpleLogPath()}`)
  ...
  private async computeSessionExportStats(
    ...
    if (includeRelations) {
      if (isGroup) {
        try {
          simpleInfo(`[computeSessionExportStats] 计算群聊 ${sessionId} 的共同好友`)
          ...
          simpleInfo(`[computeSessionExportStats] 群聊 ${sessionId} 共同好友: ${stats.groupMutualFriends}`)
        } catch (e) {
          simpleError(`[computeSessionExportStats] 计算群聊 ${sessionId} 共同好友失败:`, e)
          stats.groupMutualFriends = 0
        }
        stats.privateMutualGroups = 0
      } else {
        const allGroups = await this.listAllGroupSessionIds()
        simpleInfo(`[computeSessionExportStats] 计算私聊 ${sessionId} 的共同群聊, 总群聊数: ${allGroups.length}`)
        if (allGroups.length === 0) {
          ...
          try {
            ...
            simpleInfo(`[computeSessionExportStats] 私聊 ${sessionId} 共同群聊: ${stats.privateMutualGroups}`)
          } catch (e) {
            simpleError(`[computeSessionExportStats] 计算私聊 ${sessionId} 共同群聊失败:`, e)
            stats.privateMutualGroups = 0
          }
        }
        stats.groupMutualFriends = 0
  ...
  private async computeSessionExportStatsBatch(
    ...
    if (includeRelations) {
      ...
      if (relationGroupSessionIds.length > 0) {
        try {
          simpleInfo(`[computeSessionExportStatsBatch] 批量计算关系统计: ${relationGroupSessionIds.length}个群聊, ${privateSessionIds.length}个私聊`)
          ...
          simpleInfo(`[computeSessionExportStatsBatch] 批量计算完成`)
        } catch (e) {
          simpleError(`[computeSessionExportStatsBatch] 批量计算关系统计失败:`, e)
   ...
   * 获取当前用户的头像 URL
   */
  async getMyAvatarUrl(): Promise<{ success: boolean; avatarUrl?: string; displayName?: string; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error }
      ...
      const [avatarResult, displayNameResult] = await Promise.all([
        wcdbService.getAvatarUrls(fetchList),
        wcdbService.getDisplayNames(fetchList)
      ])

      let avatarUrl: string | undefined
      if (avatarResult.success && avatarResult.map) {
        avatarUrl = avatarResult.map[myWxid] || avatarResult.map[cleanedWxid] || avatarResult.map['self']
      }

      let displayName: string | undefined
      if (displayNameResult.success && displayNameResult.map) {
        displayName = displayNameResult.map[myWxid] || displayNameResult.map[cleanedWxid] || displayNameResult.map['self']
      }

      return { success: true, avatarUrl, displayName }
    } catch (e) {
   ...
   * @returns 性别信息：0=未知，1=男，2=女
   */
  async getContactGender(username: string): Promise<{ success: boolean; gender?: number; genderText?: string; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error }
      ...
      const result = await wcdbService.execQuery('contact', null, 
        `SELECT username, extra_buffer FROM contact WHERE username = ?`, 
        [username]
      )
      ...
      const gender = this.parseGenderFromExtraBuffer(row)
      
      const genderText = gender === 1 ? '男' : gender === 2 ? '女' : '未知'
      
      return { success: true, gender, genderText }
    } catch (e) {
      console.error('ChatService: 获取联系人性别失败:', e)
      return { success: false, error: String(e) }
    }
  }
  ...
  /**
   * 批量获取联系人的性别信息
   * @param usernames 联系人用户名数组
   * @returns 性别信息映射
   */
  async getContactsGender(usernames: string[]): Promise<{ success: boolean; genderMap?: Record<string, { gender: number; genderText: string }>; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error }
      ...
      const result = await wcdbService.execQuery('contact', null, 
        `SELECT username, extra_buffer FROM contact WHERE username IN (${placeholders})`, 
        usernames
      )
      ...
      const genderMap: Record<string, { gender: number; genderText: string }> = {}
      
      for (const row of result.rows) {
        const username = row.username
        const gender = this.parseGenderFromExtraBuffer(row)
        const genderText = gender === 1 ? '男' : gender === 2 ? '女' : '未知'
        genderMap[username] = { gender, genderText }
      }
      
      return { success: true, genderMap }
    } catch (e) {
      console.error('ChatService: 批量获取联系人性别失败:', e)
      return { success: false, error: String(e) }
    }
  }
   ...
   * 获取所有好友的性别分布统计
   */
  async getAllFriendsGenderDistribution(): Promise<{ success: boolean; distribution?: { male: number; female: number; unknown: number; total: number }; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error }
      ...
      // 获取所有非群聊联系人
      const result = await wcdbService.execQuery('contact', null, 
        `SELECT username, extra_buffer FROM contact WHERE username NOT LIKE '%@chatroom%' AND local_type = 0`
      )
      ...
      console.error('ChatService: 获取性别分布失败:', e)
      return { success: false, error: String(e) }
    }
  }
   ...
   * @returns 天气信息
   */
  async getCityWeather(city: string): Promise<{ success: boolean; weather?: string; error?: string }> {
    try {
      ...
      return new Promise((resolve, reject) => {
        const req = https.get(url, { headers: { 'User-Agent': 'curl/7.68.0' } }, (res: any) => {
          let data = ''
          res.on('data', (chunk: any) => { data += chunk })
          res.on('end', () => {
            if (data.includes('Unknown location') || data.includes('Not Found')) {
              resolve({ success: false, error: '未找到该城市的天气信息' })
            } else {
              resolve({ success: true, weather: data.trim() })
            }
          })
        })
      ...
      console.error('ChatService: 获取天气失败:', e)
      return { success: false, error: String(e) }
    }
  }
   ...
   * 获取当前用户信息
   */
  async getMyInfo(): Promise<{ success: boolean; username?: string; displayName?: string; region?: string; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error }
      ...
      // ========== 核心修复：复用与通讯录列表完全相同的数据获取路径 ==========
      // 通讯录列表（联系人详情页显示"四川 遂宁"）使用的路径是：
      //   getContactsCompact() → hydrateContactsWithExtendedFields() → getContactRegion(row)
      // 之前失败的原因：wcdbService.getContact() 在 Windows 上走原生 API，可能不返回 extra_buffer 等大字段，
      // 导致 getContactRegion() 无法从 protobuf 中提取省份/城市信息。
      // 修复方案：改用与通讯录列表相同的 compact+hydrate 路径获取完整 row 数据。

      const candidates = Array.from(new Set([myWxid, cleanedWxid].filter(Boolean)))
      const wxidCandidates = new Set<string>(
        candidates.map(c => c.toLowerCase()).concat(
          candidates.map(c => c.trim().toLowerCase())
        )
      )
      ...
      let displayName: string | undefined
      let region = ''
      let hydratedRow: Record<string, any> | null = null

      // ===== 步骤1：通过 compact + hydrate 获取包含完整 extra_buffer 的行（与通讯录列表一致）=====
      // 注意：不传 usernames 参数，这样 getContactsCompact() 返回全部联系人。
      // 之前传了 candidates 数组可能导致原生 API 过滤逻辑与预期不符。
      try {
        const compactResult = await wcdbService.getContactsCompact()
        if (compactResult.success && Array.isArray(compactResult.contacts) && compactResult.contacts.length > 0) {
          let rows = compactResult.contacts as Record<string, any>[]
          // 使用与通讯录列表完全相同的 hydrate 逻辑，确保 extra_buffer 可用
          rows = await this.hydrateContactsWithExtendedFields(rows)
          // 从 hydrate 后的行中找到自己的记录（匹配所有候选 username）
          for (const row of rows) {
            const rowUsername = String(row.username || '').trim().toLowerCase()
            if (candidates.some(c => c.toLowerCase() === rowUsername)) {
              hydratedRow = row
              break
            }
          }
        ...
        console.warn('[getMyInfo] compact+hydrate 获取失败，降级到 getContact:', e)
      }

      // ===== 步骤2：获取昵称（优先用 hydrate 行，其次用 getContact）=====
      for (const candidate of candidates) {
        let name: string | undefined

        // 优先从 hydrate 行中提取昵称（数据最完整，与联系人详情页一致）
        if (hydratedRow) {
          const hName = normalizeName(
            String(hydratedRow.remark || hydratedRow.Remark || '') ||
            String(hydratedRow.nickName || hydratedRow.nick_name || hydratedRow.nickname || hydratedRow.NickName || '') ||
            String(hydratedRow.alias || hydratedRow.Alias || '')
          )
          if (hName) name = hName
        }

        // 其次用 chatService.getContact（与 Sidebar 一致的方式）
        if (!name) {
          const contact = await this.getContact(candidate)
          if (contact?.remark || contact?.nickName || contact?.alias) {
            name = normalizeName(contact.remark) || normalizeName(contact.nickName) || normalizeName(contact.alias)
          }
        }
      ...
      if (!displayName) {
        const result = await wcdbService.execQuery('contact', null,
          ...
          [myWxid, cleanedWxid, 'self']
        )
        if (result.success && Array.isArray(result.rows) && result.rows.length > 0) {
          for (const row of result.rows) {
            // 如果还没有 hydrate 行，用 SQL 行作为备用
            if (!hydratedRow) hydratedRow = row as Record<string, any>
            const name = normalizeName(
              String(row.remark || row.Remark || '') ||
              String(row.nickName || row.nick_name || row.nickname || row.NickName || '') ||
              String(row.alias || row.Alias || '')
            )
            if (name) {
              displayName = name
              break
      ...
      // ===== 步骤3：解析地区（使用与联系人详情页相同的 getContactRegion 方法）=====
      if (hydratedRow) {
        region = this.getContactRegion(hydratedRow)
      }

      console.log('[getMyInfo] 结果:', {
        username: myWxid,
        displayName: displayName || '(未设置昵称)',
        region: region || '(未设置地区)',
        hasHydratedRow: !!hydratedRow,
        dataSource: hydratedRow ? 'compact+hydrate' : (displayName ? 'getContact' : 'none')
      })

      return {
        success: true,
        username: myWxid,
        displayName: displayName !== myWxid ? displayName : undefined,
        region: region
      }
    } catch (e) {
  ...
  /**
   * 获取与指定好友的共同群聊数量
   * @param myDisplayName 我的显示名称
   * @param friendUsername 好友用户名
   * @returns 共同群聊数量
   */
  async getCommonGroupCount(myDisplayName: string, friendUsername: string): Promise<{ success: boolean; count?: number; error?: string }> {
    try {
      const connectResult = await this.ensureConnected()
      if (!connectResult.success) {
        return { success: false, error: connectResult.error }
      ...
      // 获取当前用户微信ID
      const myUsername = this.configService.get('currentUserUsername') as string
      if (!myUsername) {
        return { success: false, error: '未找到当前用户信息' }
      }

      // 查询所有群聊
      const sessionsResult = await wcdbService.execQuery('session', null, 
        `SELECT username FROM session WHERE username LIKE '%@chatroom%'`
      )

      if (!sessionsResult.success || !Array.isArray(sessionsResult.rows)) {
        return { success: true, count: 0 }
      }

      const groupUsernames = sessionsResult.rows.map((row: any) => row.username)
      let commonCount = 0

      // 遍历每个群聊，检查是否同时包含我和好友
      for (const groupUsername of groupUsernames) {
        try {
          const membersResult = await wcdbService.execQuery('group_member', null, 
            `SELECT username FROM group_member WHERE group_username = ? AND (username = ? OR username = ?)`,
            [groupUsername, myUsername, friendUsername]
          )

          if (membersResult.success && Array.isArray(membersResult.rows)) {
            const memberSet = new Set(membersResult.rows.map((row: any) => row.username))
            if (memberSet.has(myUsername) && memberSet.has(friendUsername)) {
              commonCount++
            }
          }
        } catch (e) {
      ...
      return { success: true, count: commonCount }
    } catch (e) {
      console.error('ChatService: 获取共同群聊数量失败:', e)
      return { success: false, error: String(e) }
    }
  }
  ...
  private getVoiceCacheDir(): string {
    ...
    return join(documentsPath, 'ChatFlow', 'Voices')
  }
  ...
  private getEmojiCacheDir(): string {
    ...
    // 回退到默认目录
    const documentsPath = app.getPath('documents')
    return join(documentsPath, 'ChatFlow', 'Emojis')
  }
  ...
  async getVoiceData(sessionId: string, msgId: string, createTime?: number, serverId?: string | number, senderWxidOpt?: string): Promise<{ success: boolean; data?: string; error?: string }> {
    ...
    const verboseVoiceTrace = true // 始终启用调试日志
  ...
  async getVoiceTranscript(
    ...
    // 确保磁盘缓存已加载（异步，避免阻塞）
    await this.loadTranscriptCacheIfNeededAsync()
      ...
      const task = (async () => {
        try {
          ...
          if (wavData) {
            ...
            if (existsSync(wavFilePath)) {
              try {
                wavData = await fsPromises.readFile(wavFilePath)
  ...
  private getTranscriptCachePath(): string {
    const cachePath = this.configService.get('cachePath')
    const base = cachePath || join(app.getPath('documents'), 'ChatFlow')
    return join(base, 'Voices', 'transcripts.json')
  }

  /** 首次访问时从磁盘加载转写缓存（异步版本，避免阻塞主进程） */
  private async loadTranscriptCacheIfNeededAsync(): Promise<void> {
  ...
  /** 首次访问时从磁盘加载转写缓存（兼容旧调用） */
  private loadTranscriptCacheIfNeeded(): void {
    // 同步版本：仅标记已加载，不再同步读取文件
    // 实际加载由异步版本完成
    if (this.transcriptCacheLoaded) return
    this.transcriptCacheLoaded = true
    // 启动异步加载但不等待
    this.loadTranscriptCacheIfNeededAsync().catch(() => {})
  }
  ...
  private scheduleTranscriptFlush(): void {
    ...
    this.transcriptFlushTimer = setTimeout(() => {
      ...
      this.flushTranscriptCacheAsync().catch(() => {})
    }, 3000)
  }

  /** 异步写入转写缓存到磁盘（避免阻塞主进程） */
  private async flushTranscriptCacheAsync(): Promise<void> {
    ...
    try {
      ...
      await fsPromises.writeFile(filePath, JSON.stringify(obj), 'utf-8')
  ...
  async getChatDates(sessionIds: string[]): Promise<{ success: boolean; dates?: string[]; error?: string }> {
    try {
      ...
      const result = await wcdbService.getChatDates(sessionIds)
      if (!result.success) {
        throw new Error(result.error || '查询失败')
      }
      ...
      console.log(`[ChatService] ${sessionIds.length} 个会话共有 ${dates.length} 个有消息的日期`)
      return { success: true, dates }
    } catch (e) {
      console.error('[ChatService] 获取多个会话消息日期失败:', e)
      return { success: false, error: String(e) }
    }
  }
