...
export class ImageDecryptService {
  ...
  private getAllCacheRoots(): string[] {
    ...
    // 默认路径
    roots.push(join(documentsPath, 'ChatFlow', 'Images'))
    roots.push(join(documentsPath, 'ChatFlow', 'images'))

    // 兼容旧路径（如果有的话）
    roots.push(join(documentsPath, 'ChatFlowData', 'Images'))
  ...
  private getCacheRoot(): string {
    ...
    const root = configured
      ...
      : join(app.getPath('documents'), 'ChatFlow', 'Images')
    if (!existsSync(root)) {
  ...
  private async convertHevcToJpg(hevcData: Buffer): Promise<Buffer | null> {
    ...
    const tmpDir = join(app.getPath('temp'), 'chatflow_hevc')
