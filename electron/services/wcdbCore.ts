...
import { toNativePath } from '../utils/pathEncoding'
...
export class WcdbCore {
  ...
  startMonitor(callback: (type: string, json: string) => void): boolean {
    ...
    try {
      ...
      // 从数据服务获取动态管道名（含 PID）
      let pipePath = '\\\\.\\pipe\\chatflow_monitor'
  ...
  private getDllPath(): string {
    ...
    // 基础路径探测 - 优先尝试根目录（与CipherTalk保持一致）
    ...
    // 优先尝试根目录下的DLL（CipherTalk方式）
    const rootCandidates = [
      join(resourcesPath, 'resources', libName),  // resources/wcdb_api.dll
      join(resourcesPath, libName),               // resources/wcdb_api.dll (开发环境)
      join(process.cwd(), 'resources', libName),  // ./resources/wcdb_api.dll
    ]
    
    for (const path of rootCandidates) {
      if (existsSync(path)) {
        this.writeLog(`[bootstrap] 使用根目录DLL: ${path}`, true)
        return path
      }
    }
  ...
  private formatInitProtectionError(code: number): string {
    const messages: Record<number, string> = {
      ...
      '-2302': 'WCDB 初始化异常，请重试',
      '-2303': 'WCDB 未能成功初始化',
      '-1006': '安全验证失败，请检查应用完整性或重新安装',
      '-101': '数据库密钥无效，请重新获取密钥',
      '-102': '数据库加密验证失败，请重新获取密钥',
    }
  ...
  private writeLog(message: string, force = false): void {
    ...
    candidates.push(join(tmpdir(), 'chatflow-wcdb.log'))
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      console.log('[WCDB] 开始初始化...')
      this.koffi = require('koffi')
      console.log('[WCDB] koffi 加载成功')
      
      const dllPath = this.getDllPath()
      console.log(`[WCDB] DLL路径: ${dllPath}`)
      ...
      // 初始化
      console.log('[WCDB] 开始调用 wcdb_init()...')
      this.writeLog('[bootstrap] wcdb_init() begin', true)
      
      // 使用 Promise.race 添加超时保护
      const initWithTimeout = async (): Promise<number> => {
        return new Promise((resolve, reject) => {
          const timeoutId = setTimeout(() => {
            reject(new Error('wcdb_init() 超时 (10秒)'))
          }, 10000)
          
          try {
            const result = this.wcdbInit()
      ...
      try {
        initResult = await initWithTimeout()
      } catch (timeoutError) {
        console.error('[WCDB] wcdb_init() 调用超时或异常:', timeoutError)
        this.writeLog(`[bootstrap] wcdb_init() timeout: ${String(timeoutError)}`, true)
        lastDllInitError = 'WCDB 初始化超时，可能是 DLL 被安全软件拦截或损坏'
        return false
      }
      
      console.log(`[WCDB] wcdb_init() 返回结果: ${initResult}`)
      ...
      if (initResult !== 0) {
        ...
        this.writeLog(`[bootstrap] wcdb_init() failed: ${lastDllInitError}`, true)
        return false
      }

      console.log('[WCDB] 初始化成功')
  ...
  async testConnection(dbPath: string, hexKey: string, wxid: string): Promise<{ success: boolean; error?: string; sessionCount?: number }> {
    try {
      ...
      const handleOut = [0]
      // 转换路径编码以支持中文路径
      const nativeSessionDbPath = toNativePath(sessionDbPath)
      this.writeLog(`testConnection path encoding: "${sessionDbPath}" -> "${nativeSessionDbPath}"`)
  ...
  async open(dbPath: string, hexKey: string, wxid: string): Promise<boolean> {
    try {
      ...
      // 转换路径编码以支持中文路径
      const nativeSessionDbPath = toNativePath(sessionDbPath)
      this.writeLog(`open path encoding: "${sessionDbPath}" -> "${nativeSessionDbPath}"`, true)
      const result = this.wcdbOpenAccount(nativeSessionDbPath, hexKey, handleOut)
  ...
  async getDisplayNames(usernames: string[]): Promise<{ success: boolean; map?: Record<string, string>; error?: string }> {
    ...
    try {
      ...
      // 调试日志：打印获取到的昵称映射
      console.log('[wcdbCore.getDisplayNames] 查询结果:', { usernames, map })
  ...
  async getChatDates(sessionIds: string[]): Promise<{ success: boolean; dates?: string[]; error?: string }> {
    ...
    try {
      ...
      return { success: true, dates: Array.from(allDates) }
    } catch (e) {
      return { success: false, error: String(e) }
    }
  }
  ...
  async getMessageMeta(dbPath: string, tableName: string, limit: number, offset: number): Promise<{ success: boolean; rows?: any[]; error?: string }> {
    ...
    try {
      const outPtr = [null as any]
      // 转换路径编码以支持中文路径
      const nativeDbPath = toNativePath(dbPath)
      const result = this.wcdbGetMessageMeta(this.handle, nativeDbPath, tableName, limit, offset, outPtr)
      if (result !== 0 || !outPtr[0]) {
  ...
  async getMessageTableColumns(dbPath: string, tableName: string): Promise<{ success: boolean; columns?: string[]; error?: string }> {
    ...
    try {
      ...
      // 转换路径编码以支持中文路径
      const nativeDbPath = toNativePath(dbPath)
      const result = this.wcdbGetMessageTableColumns(this.handle, nativeDbPath, tableName, outPtr)
  ...
  async getMessageTableTimeRange(dbPath: string, tableName: string): Promise<{ success: boolean; data?: any; error?: string }> {
    ...
    try {
      ...
      // 转换路径编码以支持中文路径
      const nativeDbPath = toNativePath(dbPath)
      const result = this.wcdbGetMessageTableTimeRange(this.handle, nativeDbPath, tableName, outPtr)
  ...
  async execQuery(kind: string, path: string | null, sql: string, params: any[] = []): Promise<{ success: boolean; rows?: any[]; error?: string }> {
    ...
    try {
      ...
      const outPtr = [null as any]
      // 转换路径编码以支持中文路径
      const nativeEffectivePath = effectivePath ? toNativePath(effectivePath) : effectivePath
      const result = this.wcdbExecQuery(this.handle, kind, nativeEffectivePath, sql, outPtr)
      if (result !== 0 || !outPtr[0]) {
        if (isContactQuery) {
  ...
  async getEmoticonCdnUrl(dbPath: string, md5: string): Promise<{ success: boolean; url?: string; error?: string }> {
    ...
    try {
      ...
      // 转换路径编码以支持中文路径
      const nativeDbPath = toNativePath(dbPath)
      const result = this.wcdbGetEmoticonCdnUrl(this.handle, nativeDbPath, md5, outPtr)
      if (result !== 0 || !outPtr[0]) {
  ...
  async getEmoticonCaption(dbPath: string, md5: string): Promise<{ success: boolean; caption?: string; error?: string }> {
    ...
    try {
      const outPtr = [null as any]
      // 转换路径编码以支持中文路径
      const nativeDbPath = dbPath ? toNativePath(dbPath) : dbPath
      const result = this.wcdbGetEmoticonCaption(this.handle, nativeDbPath || '', md5, outPtr)
      if (result !== 0 || !outPtr[0]) {
  ...
  async getMediaSchemaSummary(dbPath: string): Promise<{ success: boolean; data?: any; error?: string }> {
    ...
    try {
      ...
      // 转换路径编码以支持中文路径
      const nativeDbPath = toNativePath(dbPath)
      const result = this.wcdbGetMediaSchemaSummary(this.handle, nativeDbPath, outPtr)
  ...
  async resolveImageHardlink(md5: string, accountDir?: string): Promise<{ success: boolean; data?: any; error?: string }> {
    ...
    try {
      ...
      const outPtr = [null as any]
      // 转换路径编码以支持中文路径
      const nativeAccountDir = normalizedAccountDir ? toNativePath(normalizedAccountDir) : normalizedAccountDir
      const result = this.wcdbResolveImageHardlink(this.handle, normalizedMd5, nativeAccountDir || null, outPtr)
  ...
  async resolveVideoHardlinkMd5(md5: string, dbPath?: string): Promise<{ success: boolean; data?: any; error?: string }> {
    ...
    try {
      ...
      const outPtr = [null as any]
      // 转换路径编码以支持中文路径
      const nativeDbPath = normalizedDbPath ? toNativePath(normalizedDbPath) : normalizedDbPath
      const result = this.wcdbResolveVideoHardlinkMd5(this.handle, normalizedMd5, nativeDbPath || null, outPtr)
  ...
  async resolveImageHardlinkBatch(
    ...
    try {
      ...
      // 转换路径编码以支持中文路径
      const nativeRequests = normalizedRequests.map((req) => ({
        ...req,
        accountDir: req.accountDir ? toNativePath(req.accountDir) : req.accountDir
      }))
      ...
      for (let i = 0; i < normalizedRequests.length; i += 1) {
        ...
        const nativeReq = nativeRequests[i]
        if (!req.md5) {
        ...
        if (cached) {
          ...
          unresolved.push({ index: i, md5: nativeReq.md5, accountDir: nativeReq.accountDir })
        }
      }
  ...
  async resolveVideoHardlinkMd5Batch(
    ...
    try {
      ...
      const nativeRequests = normalizedRequests.map((req) => ({
        ...req,
        dbPath: req.dbPath ? toNativePath(req.dbPath) : req.dbPath
      }))
      const rows: Array<{ index: number; md5: string; success: boolean; data?: any; error?: string }> = new Array(normalizedRequests.length)
      ...
      for (let i = 0; i < normalizedRequests.length; i += 1) {
        ...
        if (cached) {
          ...
          unresolved.push({ index: i, md5: nativeReq.md5, dbPath: nativeReq.dbPath })
        }
      }
  ...
  async searchMessages(keyword: string, sessionId?: string, limit?: number, offset?: number, beginTimestamp?: number, endTimestamp?: number): Promise<{ success: boolean; messages?: any[]; error?: string }> {
    ...
    try {
      ...
      console.log(`[WCDBCore] 搜索关键词优化: "${keyword}" -> "${optimizedKeyword}"`)

      // 如果没有指定sessionId，获取所有联系人进行全局搜索
      if (!sessionId || sessionId.trim() === '') {
        console.log(`[WCDBCore] 全局搜索模式: 搜索所有联系人`)
        return this.searchMessagesGlobal(optimizedKeyword, limit || 50, beginTimestamp || 0, endTimestamp || 0)
      }
   ...
   * 全局搜索 - 搜索所有联系人的消息
   */
  private async searchMessagesGlobal(
    keyword: string,
    limit: number,
    beginTimestamp: number,
    endTimestamp: number
    ...
    try {
      ...
      if (!sessionsResult.success || !sessionsResult.sessions) {
        console.log(`[WCDBCore] 全局搜索失败: 获取联系人列表失败 - ${sessionsResult.error}`)
        return { success: false, error: '获取联系人列表失败' }
      }

      const sessions = sessionsResult.sessions
      console.log(`[WCDBCore] 全局搜索: 共 ${sessions.length} 个联系人`)

      // 打印前几个联系人的结构，用于调试
      if (sessions.length > 0) {
        console.log(`[WCDBCore] 全局搜索: 第一个联系人结构:`, JSON.stringify(sessions[0], null, 2).substring(0, 500))
      }

      // 并行搜索每个联系人的消息（限制并发数避免阻塞）
      const batchSize = 5
      ...
      for (let i = 0; i < sessions.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (session: any) => {
          // 尝试多种可能的字段名获取sessionId
          const sessionId = session.username || session.sessionId || session.userName || session.id || session.wxid
          if (!sessionId) {
            console.log(`[WCDBCore] 全局搜索: 联系人缺少sessionId:`, session)
            return []
          }
          ...
          try {
            ...
            searchedCount++
            if (result !== 0 || !outPtr[0]) {
              console.log(`[WCDBCore] 全局搜索: 搜索 ${sessionId} 失败: result=${result}`)
              return []
            }

            const jsonStr = this.decodeJsonPtr(outPtr[0])
            if (!jsonStr) {
              console.log(`[WCDBCore] 全局搜索: 搜索 ${sessionId} 返回空结果`)
              return []
            }
            ...
            if (messages && messages.length > 0) {
              successCount++
              console.log(`[WCDBCore] 全局搜索: 搜索 ${sessionId} 找到 ${messages.length} 条消息`)
            }
            return messages || []
          } catch (e) {
            console.log(`[WCDBCore] 全局搜索: 搜索 ${sessionId} 异常: ${String(e)}`)
            return []
          }
        })
        ...
        if (allMessages.length >= limit * 2) {
          console.log(`[WCDBCore] 全局搜索: 已收集 ${allMessages.length} 条消息，提前停止`)
          break
        }
      }

      console.log(`[WCDBCore] 全局搜索完成: 搜索了 ${searchedCount} 个联系人, ${successCount} 个有结果, 共找到 ${allMessages.length} 条消息`)

      // 按时间排序并限制数量
      allMessages.sort((a, b) => (b.createTime || 0) - (a.createTime || 0))
      return { success: true, messages: allMessages.slice(0, limit) }
    } catch (e) {
      console.log(`[WCDBCore] 全局搜索异常: ${String(e)}`)
      return { success: false, error: `全局搜索失败: ${String(e)}` }
    }
  }

  /**
   * 优化搜索关键词
   * 注意：wcdb原生搜索可能不支持FTS5通配符，保持原样返回
   */
  private optimizeSearchKeyword(keyword: string): string {
    // 目前直接返回原始关键词，因为原生搜索API可能不支持通配符
    // 如果需要FTS5高级语法，可以在这里处理
  ...
  async deleteMessage(sessionId: string, localId: number, createTime: number, dbPathHint?: string): Promise<{ success: boolean; error?: string }> {
    ...
    return new Promise((resolve) => {
      try {
        const outError = [null as any]
        // 转换路径编码以支持中文路径
        const nativeDbPathHint = dbPathHint ? toNativePath(dbPathHint) : dbPathHint
        const result = this.wcdbDeleteMessage(this.handle, sessionId, localId, createTime || 0, nativeDbPathHint || '', outError)
