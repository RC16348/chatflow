...
export class VoiceTranscribeService {
  ...
  private buildTranscribeWorkerEnv(): NodeJS.ProcessEnv {
    ...
    // 获取应用根路径（打包后在 resources/app.asar.unpacked/，开发时在项目根目录）
    const appPath = app.getAppPath()
    const isPackaged = appPath.includes('app.asar')
    const unpackedPath = isPackaged ? appPath.replace(/app\.asar$/, 'app.asar.unpacked') : appPath

    // 构建 sherpa-onnx 平台包的可能路径
    const candidates = [
      join(unpackedPath, 'node_modules', platformPkg),
    ...
    // 构建 node_modules 的可能路径（用于 NODE_PATH）
    const nodeModulesPaths = [
      join(unpackedPath, 'node_modules'),
    ...
    console.log('[VoiceTranscribe] Worker node_modules 搜索路径:', nodeModulesPaths)
    console.log('[VoiceTranscribe] Worker platformPkg 搜索路径:', candidates)

    // 设置 NODE_PATH 让 worker 能找到 sherpa-onnx-node 模块
    const separator = process.platform === 'win32' ? ';' : ':'
    const existingNodePath = env.NODE_PATH || ''
    const mergedNodePath = [...nodeModulesPaths, ...existingNodePath.split(separator).filter(Boolean)]
    env.NODE_PATH = Array.from(new Set(mergedNodePath)).join(separator)

    // 同时设置 ELECTRON_RUN_AS_NODE=1 以确保 worker 能正确加载原生模块
  ...
  private resolveModelDir(): string {
    ...
    return join(app.getPath('documents'), 'ChatFlow', 'models', 'sensevoice')
  }
