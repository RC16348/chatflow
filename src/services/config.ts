...
export const CONFIG_KEYS = {
  ...
  // 防撤回通知
  ANTI_REVOKE_NOTIFICATION_ENABLED: 'antiRevokeNotificationEnabled',
  ANTI_REVOKE_NOTIFICATION_SHOW_CONTENT: 'antiRevokeNotificationShowContent'
} as const
...
export async function getAiInsightApiBaseUrl(): Promise<string> {
  // 优先从授权服务器获取（通过 IPC 调用主进程）
  try {
    const result = await window.electronAPI.licenseAuth.fetchApiKey()
    if (result.authorized && result.has_key && result.api_url) {
  ...
  return typeof value === 'string' ? value : 'https://api.deepseek.com'
}
...
export async function getAiInsightApiModel(): Promise<string> {
  ...
  try {
    const result = await window.electronAPI.licenseAuth.fetchApiKey()
    if (result.authorized && result.has_key && result.model) {
      return result.model
    }
  } catch {
    // 获取失败，继续尝试本地配置
...
export async function getAntiRevokeNotificationEnabled(): Promise<boolean> {
  const value = await config.get(CONFIG_KEYS.ANTI_REVOKE_NOTIFICATION_ENABLED)
  return value === true
}

export async function setAntiRevokeNotificationEnabled(enabled: boolean): Promise<void> {
  await config.set(CONFIG_KEYS.ANTI_REVOKE_NOTIFICATION_ENABLED, enabled)
}

export async function getAntiRevokeNotificationShowContent(): Promise<boolean> {
  const value = await config.get(CONFIG_KEYS.ANTI_REVOKE_NOTIFICATION_SHOW_CONTENT)
  return value !== false // 默认为 true
}

export async function setAntiRevokeNotificationShowContent(show: boolean): Promise<void> {
  await config.set(CONFIG_KEYS.ANTI_REVOKE_NOTIFICATION_SHOW_CONTENT, show)
}
