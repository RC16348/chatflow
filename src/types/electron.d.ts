...
export interface ElectronAPI {
  ...
  dbPath: {
    ...
    onAutoDetectProgress: (callback: (progress: { stage: string; currentPath: string; scannedCount: number; foundCount: number; percentage: number }) => void) => () => void
  }
  wcdb: {
  ...
  chat: {
    ...
    testAntiRevokeNotification: () => Promise<{ success: boolean; message?: string; error?: string }>
    checkSessionForRevoke: (sessionId: string, summary: string, timestamp: number, displayName: string) => Promise<{ success: boolean }>
    ...
    getContactGender: (username: string) => Promise<{
      success: boolean
      gender?: number
      genderText?: string
      error?: string
    }>
    getContactsGender: (usernames: string[]) => Promise<{
      success: boolean
      genderMap?: Record<string, { gender: number; genderText: string }>
      error?: string
    }>
    getAllFriendsGenderDistribution: () => Promise<{
      success: boolean
      distribution?: { male: number; female: number; unknown: number; total: number }
      error?: string
    }>
    getMyInfo: () => Promise<{
      success: boolean
    ...
    getCommonGroupCount: (myDisplayName: string, friendUsername: string) => Promise<{
      success: boolean
      count?: number
      error?: string
    }>
    getMyAvatarUrl: () => Promise<{ success: boolean; avatarUrl?: string; displayName?: string; error?: string }>
    ...
    getChatDates: (sessionIds: string[]) => Promise<{ success: boolean; dates?: string[]; error?: string }>
    ...
    getVoiceTranscript: (sessionId: string, msgId: string, createTime?: number, senderWxid?: string) => Promise<{ success: boolean; transcript?: string; error?: string }>
  ...
  licenseAuth: {
    ...
    getMachineUUID: () => Promise<{ success: boolean; uuid?: string; error?: string }>
    ...
    clearApiKeyCache: () => Promise<{ success: boolean }>
  }
  agent: {
    ...
    createConversation: (convId: string, title: string) => Promise<{
      id: string
      title: string
      createdAt: number
      updatedAt: number
    ...
    saveMessage: (convId: string, message: any) => Promise<{ success: boolean }>
    deleteConversation: (convId: string) => Promise<boolean>
    clearAllData: () => Promise<boolean>
    executeTool: (toolName: string, args: any) => Promise<any>
    appendAgentLog: (sessionId: string, content: string) => Promise<{ success: boolean; error?: string }>
  }
  insight: {
    testConnection: () => Promise<{ success: boolean; message: string }>
    getTodayStats: () => Promise<{ sessionId: string; count: number; times: string[] }[]>
