...
import { Check, Download, FileText, Image, SlidersHorizontal, X } from 'lucide-react'
...
function DualReportWindow() {
  ...
  const sectionRefs = {
    ...
    aiInsight: useRef<HTMLElement>(null),
  ...
  useEffect(() => {
    const loadEmojis = async () => {
      ...
      // 尝试加载我的常用表情
      if (stats.myTopEmojiMd5) {
        // 优先使用已有的 URL，否则尝试从微信 CDN 构造 URL
        const emojiUrl = stats.myTopEmojiUrl || `https://emoji.qpic.cn/wx_emoji/${stats.myTopEmojiMd5}/0`
        const res = await window.electronAPI.chat.downloadEmoji(emojiUrl, stats.myTopEmojiMd5)
      ...
      // 尝试加载对方的常用表情
      if (stats.friendTopEmojiMd5) {
        const emojiUrl = stats.friendTopEmojiUrl || `https://emoji.qpic.cn/wx_emoji/${stats.friendTopEmojiMd5}/0`
        const res = await window.electronAPI.chat.downloadEmoji(emojiUrl, stats.friendTopEmojiMd5)
        if (res.success && res.localPath) {
  ...
  const getAvailableSections = (): SectionInfo[] => {
    ...
    sections.push({ id: 'aiInsight', name: 'AI 关系洞察', ref: sectionRefs.aiInsight })
  ...
  const generateAiInsight = async () => {
    ...
    try {
      ...
      const [apiUrl, apiKey, model] = await Promise.all([
        config.getAiInsightApiBaseUrl(),
        config.getAiInsightApiKey(),
        config.getAiInsightApiModel()
      ])

      if (!apiUrl || !apiKey) {
        setAiInsight('AI 服务未配置，无法生成关系洞察。')
        return
      }

      const totalMessages = reportData.stats.totalMessages
      const initiativeData = reportData.initiative
      const initiated = initiativeData?.initiated || 0
      const received = initiativeData?.received || 0
      const initiativeTotal = initiated + received
      const initiationRate = initiativeTotal > 0 ? Math.round((initiated / initiativeTotal) * 100) : 50
      const activeTime = (() => {
        if (!reportData.heatmap) return null
        let maxHour = 0
        let maxWeekday = 0
        let maxVal = -1
        reportData.heatmap.forEach((row, weekday) => {
      ...
      const peakHourStr = activeTime
        ? `${activeTime.weekday} ${String(activeTime.hour).padStart(2, '0')}:00`
        : '未知'
      const maxStreakDays = reportData.streak?.days || 0
      const avgReplyTime = reportData.response ? `${Math.round(reportData.response.avg / 60)}分钟` : '未知'

      // 获取双方性别、地区和共同群聊信息
      let auxiliaryInfo = ''
      try {
        const friendUsername = reportData.friendUsername
        
        // 获取双方性别
        const genderResult = await window.electronAPI.chat.getContactsGender([friendUsername])
        ...
        const friendContact = contactsResult.contacts?.find((c: any) => c.username === friendUsername)
        
        // 获取共同群聊数量
        const commonGroupsResult = await window.electronAPI.chat.getCommonGroupCount(reportData.selfName, friendUsername)
        
        const genderMap: Record<number, string> = { 0: '未知', 1: '男', 2: '女' }
        const myGender = myInfoResult.success ? genderMap[0] : '未知' // 暂时无法获取自己性别
        const friendGender = genderResult.success && genderResult.genderMap?.[friendUsername] 
          ? genderResult.genderMap[friendUsername].genderText 
          : '未知'
        
        const friendRegion = friendContact?.region || ''
        const commonGroupsCount = commonGroupsResult.success ? commonGroupsResult.count : 0
        
        const infoParts: string[] = []
        if (friendGender !== '未知') {
          infoParts.push(`对方性别：${friendGender}`)
        }
        if (friendRegion) {
          infoParts.push(`对方地区：${friendRegion}`)
        }
        if (commonGroupsCount && commonGroupsCount > 0) {
          infoParts.push(`共同群聊：${commonGroupsCount}个`)
        }
        ...
        console.warn('获取辅助信息失败:', e)
      }

      const prompt = `你是一位专业的关系分析师。请根据以下聊天统计数据，分析这两人的关系特点，给出简洁有温度的洞察（150字以内）：

聊天统计：
- 总消息数: ${totalMessages}
- 主动发起比例: 我 ${initiationRate}% / 对方 ${100 - initiationRate}%
- 最活跃时段: ${peakHourStr}
- 最长连续聊天: ${maxStreakDays} 天
- 平均回复时间: ${avgReplyTime}${auxiliaryInfo}
...
3. 一个有趣的发现`
      ...
      const response = await fetch(`${apiUrl}/v1/chat/completions`, {
        ...
        body: JSON.stringify({
          model: model || '',
          messages: [
            { role: 'system', content: '你是一位专业的关系分析师，善于从数据中发现关系的特点。回答简洁客观，基于事实分析，避免空洞套话和过度解读。' },
      ...
      const text = data.choices?.[0]?.message?.content || '分析生成失败，请稍后重试。'
      ...
      setAiInsight('AI 分析失败，请检查网络连接后重试。')
    } finally {
  ...
  const AiInsightSection = () => {
    useEffect(() => {
      if (!aiInsight && !isLoadingAiInsight) {
        void generateAiInsight()
      }
    }, [])

    return (
      <section className="section" ref={sectionRefs.aiInsight}>
        <div className="label-text">AI 洞察</div>
        <h2 className="hero-title">AI 关系洞察</h2>
        <div style={{ padding: '40px 20px', textAlign: 'center' }}>
          {isLoadingAiInsight ? (
            <div style={{ color: 'var(--text-secondary, #999)' }}>
              <div className="export-spinner" style={{ margin: '0 auto 16px' }}>
            ...
            <div style={{ color: 'var(--text-secondary, #999)', fontSize: '16px' }}>
              AI 服务未配置，无法生成关系洞察
            </div>
          )}
        </div>
      ...
      <div className="report-scroll-view">
        <div className="report-container" ref={containerRef}>
          <section className="section" ref={sectionRefs.cover}>
            <div className="label-text">CHATFLOW · DUAL REPORT</div>
