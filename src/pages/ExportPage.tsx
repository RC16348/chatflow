...
type TextExportFormat = 'json' | 'arkme-json' | 'html' | 'txt' | 'excel'
...
function ExportPage() {
  ...
  const [loadingMessageDates, setLoadingMessageDates] = useState(false)
  ...
  const [exportDefaultDateRangePreset, setExportDefaultDateRangePreset] = useState<ExportDateRangePreset>('today')
  ...
  // 辅助函数：根据当前预设动态创建日期范围选择
  const getDefaultDateRangeSelection = useCallback((): ExportDateRangeSelection => {
    // 如果预设是 'custom'，则使用 'today' 作为默认值
    const preset = exportDefaultDateRangePreset === 'custom' ? 'today' : exportDefaultDateRangePreset
    return createExportDateRangeSelectionFromPreset(preset)
  }, [exportDefaultDateRangePreset])
  ...
  const loadBaseConfig = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      // 从配置中解析预设类型，如果是自定义范围则默认使用 'today'
      const resolvedDefaultDateRange = resolveExportDateRangeConfig(savedDefaultDateRange)
      const preset = resolvedDefaultDateRange.preset === 'custom' ? 'today' : resolvedDefaultDateRange.preset
      setExportDefaultDateRangePreset(preset)
  ...
  const openExportDialog = useCallback((payload: Omit<ExportDialogState, 'open'>) => {
    ...
    // 动态创建当前时间的日期范围选择，避免使用过期的日期
    const defaultSelection = getDefaultDateRangeSelection()
    setTimeRangeSelection(defaultSelection)

    setOptions(prev => {
      const nextDateRange = cloneExportDateRange(defaultSelection.dateRange)

      const next: ExportOptions = {
        ...prev,
        format: exportDefaultFormat,
        exportAvatars: exportDefaultAvatars,
        useAllTime: defaultSelection.useAllTime,
  ...
  const openTimeRangeDialog = useCallback(() => {
    void (async () => {
      ...
      try {
        ...
        if (exportDialog.scope !== 'sns' && exportDialog.sessionIds.length > 0) {
          const dateResult = await window.electronAPI.chat.getChatDates(exportDialog.sessionIds)
          if (dateResult.success && dateResult.dates) {
            setMessageDates(new Set(dateResult.dates))
  ...
  const handleExportDefaultsChanged = useCallback((patch: ExportDefaultsSettingsPatch) => {
    ...
    if (patch.dateRange) {
      // 只保存预设类型，不保存完整的日期范围
      const preset = patch.dateRange.preset === 'custom' ? 'today' : patch.dateRange.preset
      setExportDefaultDateRangePreset(preset)
    }
  ...
  return (
    <div className="export-board-page">
      ...
      <div className="export-main-content">
        ...
        {/* 右侧：按会话导出 */}
        <div className="export-session-section">
          <div className="export-section-title-row">
        ...
        <div className="export-dialog-overlay" onClick={closeExportDialog}>
          <div className="export-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="dialog-body">
              {exportDialog.scope !== 'single' && exportDialog.scope !== 'sns' && (
            ...
            <ExportDateRangeDialog
              ...
              loadingMessageDates={loadingMessageDates}
