...
import { Users, BarChart3, Clock, Image, Loader2, RefreshCw, Medal, Search, X, ChevronLeft, Copy, Check, Download, ChevronDown, MessageSquare, Calendar, PieChart, Hash, Smile, Camera } from 'lucide-react'
...
type MemberExportFormat = 'chatlab' | 'chatlab-jsonl' | 'json' | 'arkme-json' | 'html' | 'txt' | 'excel'
...
function GroupAnalyticsPage() {
  ...
  // 排行榜截图
  const [isCapturingRanking, setIsCapturingRanking] = useState(false)
  ...
  const captureRanking = async () => {
    const element = rankingListRef.current
    if (!element || rankings.length === 0) return

    // 确保元素有有效尺寸
    const rect = element.getBoundingClientRect()
    if (rect.width === 0 || rect.height === 0) {
      console.error('元素尺寸为0，无法截图')
      return
    }
    ...
    try {
      ...
      await drawWatermark({ ctx, width: canvas.width, height: canvas.height, isDark: false })

      const link = document.createElement('a')
      const groupName = selectedGroup?.displayName || selectedGroup?.username || '群聊'
      link.download = `群聊发言排行_${groupName}.png`
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <button
                      ...
                      disabled={isCapturingRanking || rankings.length === 0}
                      style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px', fontSize: '13px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-tertiary)', color: 'var(--text-primary)', cursor: 'pointer' }}
                    >
                      {isCapturingRanking ? <Loader2 size={14} className="spin" /> : <Camera size={14} />}
                      <span>{isCapturingRanking ? '截图中...' : '截图分享'}</span>
                    </button>
                  </div>
                  <div className="rankings-list" ref={rankingListRef}>
