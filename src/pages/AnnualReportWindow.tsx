...
import { Loader2, Download, FileText, Image, Check, X, SlidersHorizontal } from 'lucide-react'
...
function AnnualReportWindow() {
  ...
  const exportSection = async (section: SectionInfo): Promise<{ name: string; data: string } | null> => {
    ...
    try {
      ...
      const origCreatePattern2 = CanvasRenderingContext2D.prototype.createPattern
      CanvasRenderingContext2D.prototype.createPattern = function (this: any, image: any, repetition: any) {
        if (image instanceof HTMLCanvasElement && (image.width === 0 || image.height === 0)) {
          const dummy = document.createElement('canvas')
          dummy.width = 1
          dummy.height = 1
          return origCreatePattern2.call(this, dummy, repetition)
        }
        return origCreatePattern2.call(this, image, repetition)
      } as any

      let canvas2: HTMLCanvasElement
      try {
        canvas2 = await html2canvas(element, {
        ...
        CanvasRenderingContext2D.prototype.createPattern = origCreatePattern2
      }
      ...
      await drawWatermark({ ctx, width: OUTPUT_WIDTH, height: OUTPUT_HEIGHT, isDark })
      ...
      // 计算缩放和居中位置
      const srcRatio = canvas2.width / canvas2.height
      ...
      ctx.drawImage(canvas2, drawX, drawY, drawWidth, drawHeight)
    ...
    try {
      ...
      CanvasRenderingContext2D.prototype.createPattern = function (this: any, image: any, repetition: any) {
        if (image instanceof HTMLCanvasElement && (image.width === 0 || image.height === 0)) {
          ...
          dummy.width = 1
          dummy.height = 1
          return origCreatePattern.call(this, dummy, repetition)
        }
      ...
      await drawWatermark({ ctx, width: canvas.width, height: canvas.height, isDark })
      ...
      <div className="report-scroll-view">
        <div className="report-container" ref={containerRef}>
          ...
          <section className="section" ref={sectionRefs.cover}>
            <div className="label-text">CHATFLOW · ANNUAL REPORT</div>
          ...
          <section className="section" ref={sectionRefs.overview}>
            ...
            {(reportData.sentMessages !== undefined && reportData.receivedMessages !== undefined) && (
              <p className="hero-desc">
                其中你发送了 <span className="hl">{formatNumber(reportData.sentMessages)}</span> 条，
                收到了 <span className="hl">{formatNumber(reportData.receivedMessages)}</span> 条。
              </p>
            )}
            {reportData.messageTypeStats && (
              <div className="message-type-stats">
                <div className="type-row"><span className="type-label">📝 文字</span><span className="type-count">{formatNumber(reportData.messageTypeStats.text)}</span></div>
                <div className="type-row"><span className="type-label">🖼️ 图片</span><span className="type-count">{formatNumber(reportData.messageTypeStats.image)}</span></div>
                <div className="type-row"><span className="type-label">🎤 语音</span><span className="type-count">{formatNumber(reportData.messageTypeStats.voice)}</span></div>
                <div className="type-row"><span className="type-label">🎬 视频</span><span className="type-count">{formatNumber(reportData.messageTypeStats.video)}</span></div>
                <div className="type-row"><span className="type-label">😊 表情</span><span className="type-count">{formatNumber(reportData.messageTypeStats.emoji)}</span></div>
                <div className="type-row"><span className="type-label">🔗 链接</span><span className="type-count">{formatNumber(reportData.messageTypeStats.link)}</span></div>
              </div>
            )}
            <p className="hero-desc">
          ...
          {topFriend && (
            <section className="section" ref={sectionRefs.bestFriend}>
              ...
              <p className="hero-desc">
                你发送了 <span className="hl">{formatNumber(topFriend.sentCount)}</span> 条 ·
                收到对方 <span className="hl">{formatNumber(topFriend.receivedCount)}</span> 条
              </p>
              <br />
              <p className="hero-desc">
                在一起，就可以
          ...
          {mutualFriend && (
            <section className="section" ref={sectionRefs.mutualFriend}>
              ...
              <div className="mutual-visual">
                <div className="mutual-side you">
                  ...
                  <div className="mutual-arrow">
                    <span className="arrow-label">我发给TA</span>
                ...
                <div className="mutual-side friend">
                  <div className="mutual-arrow reverse">
                    <span className="arrow-label">TA发给我</span>
              ...
              <p className="hero-desc">
                你们的互动比例接近 <span className="hl">{mutualFriend.ratio}</span>（你发{mutualFriend.sentCount}条 : 对方发{mutualFriend.receivedCount}条）。
                <br />你来我往，势均力敌。
              </p>
            </section>
          )}
          ...
          {socialInitiative && (
            <section className="section" ref={sectionRefs.socialInitiative}>
              ...
              <div className="big-stat">
                ...
                <span className="stat-unit">的对话由你主动开启</span>
              </div>
              <p className="hero-desc">
                这一年，你主动开启了 <span className="hl">{socialInitiative.initiatedChats}</span> 次对话，
                <br />收到好友主动开启的 <span className="hl">{socialInitiative.receivedChats}</span> 次对话。
              </p>
            </section>
          )}
          ...
          {peakDay && (
            <section className="section" ref={sectionRefs.peakDay}>
              ...
              <p className="hero-desc">这一天，你们的对话格外热烈</p>
              <div className="big-stat">
                <span className="stat-num">{formatNumber(peakDay.messageCount)}</span>
                <span className="stat-unit">条消息</span>
              </div>
              <p className="hero-desc">
                其中与 <span className="hl">{peakDay.topFriend || '好友'}</span> 的对话最多
                <br />共 {formatNumber(peakDay.topFriendCount || 0)} 条消息
                <br />那一天，一定有特别的故事
              </p>
            </section>
          )}
          ...
          {reportData.monthlyTrend && reportData.monthlyTrend.length > 0 && (
            <section className="section">
              <div className="label-text">月度趋势</div>
              <h2 className="hero-title">消息变化</h2>
              <div className="monthly-trend-chart">
                {reportData.monthlyTrend.map(m => {
                  const total = m.sent + m.received
                  const maxTotal = Math.max(...reportData.monthlyTrend!.map(x => x.sent + x.received), 1)
                  const heightPercent = Math.max((total / maxTotal) * 100, 4)
                  const sentPercent = total > 0 ? Math.round((m.sent / total) * 100) : 50
                  return (
                    <div key={m.month} className="trend-bar-wrapper">
                      <div className="trend-bar" style={{ height: `${heightPercent}%` }}>
                        <div className="trend-bar-sent" style={{ height: `${sentPercent}%` }} />
                        <div className="trend-bar-received" style={{ height: `${100 - sentPercent}%` }} />
                      </div>
                      <div className="trend-label">{m.month}月</div>
                      <div className="trend-count">{formatNumber(total)}</div>
                    </div>
                  )
                })}
              </div>
              <div className="trend-legend">
                <span className="legend-item"><span className="legend-dot sent" /> 你发送</span>
                <span className="legend-item"><span className="legend-dot received" /> 对方发送</span>
              </div>
            </section>
          )}
          ...
          {midnightKing && (
            <section className="section" ref={sectionRefs.midnightKing}>
              <div className="label-text">深夜互动</div>
              <h2 className="hero-title">月光下的对话</h2>
              <p className="hero-desc">深夜时段(00:00-06:00)，你与 <span className="hl">{midnightKing.displayName}</span> 互动最多</p>
              <div className="big-stat">
                <span className="stat-num">{midnightKing.count}</span>
                <span className="stat-unit">条深夜消息</span>
              </div>
              <p className="hero-desc">
                占你全部深夜聊天的 <span className="gold">{midnightKing.percentage}%</span>
              </p>
            </section>
          )}
          ...
          {responseSpeed && (
            <section className="section" ref={sectionRefs.responseSpeed}>
              ...
              <div className="big-stat">
                ...
                <span className="stat-unit">是你回复好友的平均时间</span>
              </div>
              <p className="hero-desc">
                你对 <span className="hl">{responseSpeed.fastestFriend}</span> 的回复最为迅速
          ...
          {lostFriend && (
            <section className="section" ref={sectionRefs.lostFriend}>
              ...
              <p className="hero-desc">
                ...
                <br />你们曾热烈交流，共 {formatNumber(lostFriend.earlyCount)} 条消息
                <br />后来只剩下 {formatNumber(lostFriend.lateCount)} 条
              </p>
              <div className="lost-friend-visual">
                <div className="avatar-group sender">
                  ...
                  <span className="avatar-label">{lostFriend.displayName}</span>
                </div>
            ...
            <div className="podium">
              ...
              {coreFriends[1] && (
                <div className="podium-item second">
                  ...
                  <div className="podium-detail">你发 {formatNumber(coreFriends[1].sentCount)} · 对方发 {formatNumber(coreFriends[1].receivedCount)}</div>
              ...
              {coreFriends[0] && (
                <div className="podium-item first">
                  ...
                  <div className="podium-detail">你发 {formatNumber(coreFriends[0].sentCount)} · 对方发 {formatNumber(coreFriends[0].receivedCount)}</div>
                  <div className="podium-stand">
              ...
              {coreFriends[2] && (
                <div className="podium-item third">
                  ...
                  <div className="podium-detail">你发 {formatNumber(coreFriends[2].sentCount)} · 对方发 {formatNumber(coreFriends[2].receivedCount)}</div>
                  <div className="podium-stand">
          ...
          <section className="section ending" ref={sectionRefs.ending}>
            ...
            <div className="ending-brand">CHATFLOW</div>
          </section>
        </div>
      </div>
    </div>
