...
export function GlobalSessionMonitor() {
    ...
    const checkForNewMessages = async (oldSessions: ChatSession[], newSessions: ChatSession[]) => {
        ...
        for (const newSession of newSessions) {
            ...
            // 检查是否是撤回消息（优先处理）
            if (oldSession && newSession.lastTimestamp > oldSession.lastTimestamp) {
                const summary = newSession.summary || ''
                // 检查摘要是否包含撤回相关文本
                if (/撤回了一条消息|尝试撤回此消息|recall.*message|revoke.*message/i.test(summary)) {
                    // 调用主进程检查防撤回通知
                    window.electronAPI.chat?.checkSessionForRevoke?.(
                        newSession.username,
                        summary,
                        newSession.lastTimestamp,
                        newSession.displayName || newSession.username
                    )
                }
            }
