...
export function ExportDateRangeDialog({
  ...
  const hasMessage = useCallback((date: Date): boolean => {
    if (!hasLoadedMessageDates) return true
    if (!messageDates || messageDates.size === 0) return false
    return messageDates.has(formatDateKey(date))
  }, [messageDates, hasLoadedMessageDates])

  const isDateSelectable = useCallback((date: Date) => {
    if (!bounds) return true
    const target = startOfDay(date).getTime()
    const inBounds = target >= startOfDay(bounds.minDate).getTime() && target <= startOfDay(bounds.maxDate).getTime()
    if (!inBounds) return false
    if (!hasLoadedMessageDates) return true
    return hasMessage(date)
  }, [bounds, hasLoadedMessageDates, hasMessage])
        ...
        <section className="export-date-range-calendar-panel single">
          ...
          <div className="export-date-range-calendar-days">
            {calendarCells.map((cell) => {
              ...
              const hasMsg = hasMessage(cell.date)
              const isNoMessage = hasLoadedMessageDates && !hasMsg
              return (
                <button
