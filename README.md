# ChatFlow

<p align="center">
  <img src="public/icon.png" width="120" alt="ChatFlow Logo">
</p>

<p align="center">
  <b>微信聊天记录分析工具</b> | <b>开源透明</b> | <b>本地处理</b>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-2.0.1-blue" alt="Version 2.0.1">
  <img src="https://img.shields.io/badge/license-CC%20BY--NC--SA%204.0-green" alt="License">
</p>

<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#安装使用">安装使用</a> •
  <a href="#构建指南">构建指南</a> •
  <a href="#技术架构">技术架构</a> •
  <a href="#开源协议">开源协议</a>
</p>

---

## 📖 项目简介

ChatFlow 是一款开源的微信聊天记录分析工具，支持查看、搜索、导出和分析微信聊天记录。所有数据处理均在本地完成，保护用户隐私。

**核心特点：**
- 🔒 **完全本地处理** - 聊天记录不会上传到任何服务器
- 📊 **强大的分析功能** - 年度报告、双人报告、群聊分析
- 🤖 **AI 助手** - 支持多种大语言模型，智能分析聊天记录
- 🚀 **防撤回** - 实时监测并记录撤回的消息
- 🪟 **Windows 专属** - 专为 Windows 10+ 优化

## ✨ 功能特性

### 界面预览

<p align="center">
  <img src="public/screenshot1.png" width="80%" alt="AI助手对话界面">
  <br>
  <i>AI 助手 - 智能对话与天气查询</i>
</p>

<p align="center">
  <img src="public/screenshot2.png" width="80%" alt="AI助手首页">
  <br>
  <i>AI 助手 - 快捷功能入口</i>
</p>

### 基础功能
- [x] 微信聊天记录查看与搜索
- [x] 联系人管理
- [x] 朋友圈查看
- [x] 图片、视频、语音消息支持

### 导出功能
- [x] HTML 导出（保留样式和多媒体）
- [x] TXT 导出
- [x] CSV 导出
- [x] 批量导出支持
- [x] **人物 Skill 导出** - 将聊天记录导出为 AI 可学习的 Skill 格式（支持 OpenClaw、Claude Code 等格式）

### 分析功能
- [x] 年度聊天报告
- [x] 双人聊天报告
- [x] 群聊数据分析
- [x] 聊天词云
- [x] 消息热力图

### AI 功能

#### AI 助手（Agent）
基于 ReAct 框架的智能对话助手，支持多种大语言模型（DeepSeek、OpenAI 等），具备 30+ 专业分析工具：

**📝 聊天分析工具**
- **聊天记录总结** - 智能总结单聊/群聊的聊天内容
- **群聊角色分析** - 分析群成员的发言特点和角色定位
- **聊天风格画像** - 分析个人的聊天习惯、用语风格
- **情感日历** - 可视化展示聊天频率和情感波动
- **消息分类标记** - 自动分类标记不同类型的消息

**💬 对话辅助工具**
- **智能搜索** - 通过自然语言搜索历史聊天记录
- **回复建议** - 根据上下文提供智能回复建议
- **话题开启** - 为久未联系的好友提供开场白建议
- **语音消息总结** - 自动转录并总结语音消息内容

**❤️ 关系分析工具**
- **关系健康度评分** - 基于聊天数据评估关系状态
- **关系时间线** - 梳理关系发展的关键节点
- **纪念日发现** - 智能识别特殊日子和重要时刻
- **承诺追踪** - 记录和追踪聊天中的约定事项

**🌍 实用信息工具**
- **天气查询** - 获取指定城市的实时天气
- **联系人地区分布** - 分析好友的地理分布
- **性别分布统计** - 统计好友性别比例
- **个性签名解析** - 分析联系人的微信签名

**👁️ AI 见解（后台智能分析）**
在后台默默分析聊天数据，适时通过弹窗提供智能建议：
- **久未联系提醒** - 提醒你长时间未联系的好友
- **对话回复建议** - 针对当前对话提供回复建议
- **白名单控制** - 可设置仅对特定联系人启用
- **本地分析** - 所有分析均在本地完成，保护隐私
- **灵活配置** - 支持设置冷却时间、扫描间隔等参数

### 高级功能
- [x] 消息防撤回通知
- [x] 语音转文字
- [x] 图片解密查看

## 📦 安装使用

### 下载预编译版本

前往 [Releases](https://github.com/RC16348/chatflow/releases) 页面下载对应平台的安装包。

> **注意**：目前仅在 Windows 10+ 平台上进行开发和测试，提供 Windows 版本的预编译安装包。

### 从源码构建

#### 环境要求

- **操作系统**: Windows 10 或更高版本
- **Node.js**: 18.0.0 或更高版本
- **Visual Studio Build Tools** 或 Visual Studio 2019+（用于编译原生模块）

#### 构建步骤

```bash
# 克隆仓库
git clone https://github.com/RC16348/chatflow.git
cd chatflow

# 安装依赖
npm install

# 开发模式运行
npm run dev

# 构建 Windows 版本
npm run build
```

> **说明**：本项目主要在 Windows 10+ 环境下开发和测试，其他平台的兼容性需要开发者自行验证和适配。

## 🏗️ 技术架构

### 技术栈

- **前端**: React 19 + TypeScript + Vite
- **桌面框架**: Electron
- **状态管理**: Zustand
- **样式**: SCSS
- **数据库**: SQLite (WCDB 解密)
- **构建**: electron-builder

### 项目结构

```
chatflow/
├── src/                    # 前端源码
│   ├── components/         # React 组件
│   ├── pages/             # 页面组件
│   ├── services/          # 业务服务
│   ├── stores/            # 状态管理
│   ├── utils/             # 工具函数
│   └── types/             # 类型定义
├── electron/              # Electron 主进程
│   ├── services/          # 主进程服务
│   ├── windows/           # 窗口管理
│   └── main.ts            # 入口文件
└── resources/             # 静态资源
```

### 核心模块

| 模块 | 说明 | 路径 |
|------|------|------|
| WCDB 解密 | 微信数据库解密核心 | `electron/services/wcdbService.ts` |
| 消息解析 | 微信消息格式解析 | `src/utils/messageParser.ts` |
| 导出服务 | 多格式导出实现 | `electron/services/exportService.ts` |
| AI Agent | 智能助手引擎 | `src/services/agent/` |

## 📄 开源协议

本项目采用 [CC BY-NC-SA 4.0](LICENSE)（知识共享 署名-非商业性使用-相同方式共享 4.0 国际版）协议开源。

### 授权范围

✅ **允许**
- 个人学习、研究使用
- 修改、二次开发
- 在非商业项目中使用
- 分享、传播本项目

❌ **禁止**
- 商业使用（包括但不限于销售、商业服务）
- 修改后闭源
- 更换协议后分发

## ⚠️ 免责声明

1. 本项目仅供学习研究使用，请遵守相关法律法规
2. 使用本工具产生的任何数据请妥善保管，避免泄露
3. 请仅分析您拥有合法权限的聊天记录

## 🙏 致谢与参考

本项目基于 [WeFlow](https://github.com/hicccc77/WeFlow) 进行二次开发，在保留其核心功能的基础上，进行了代码重构、功能增强和个性化扩展。

### 主要参考项目

- **[WeFlow](https://github.com/hicccc77/WeFlow)** - 基础框架来源
  - 基于 WeFlow 的代码进行二次开发
  - 保留了微信数据库解密和消息解析等核心功能
  - 在此基础上进行了代码重构和功能增强
  - 添加了 AI 助手、防撤回通知等个性化功能

- **[ChatLab](https://github.com/ChatLab/ChatLab)** - 功能设计参考
  - 借鉴了其 AI 助手与聊天记录结合的设计理念
  - 参考了隐私优先的本地数据处理方案

### 致谢说明

感谢 [WeFlow](https://github.com/hicccc77/WeFlow) 项目提供的优秀基础框架，以及 [ChatLab](https://github.com/ChatLab/ChatLab) 项目的设计灵感。本项目在开源社区的基础上进行了二次开发和创新，添加了更多符合个人需求的功能特性。

## 👍 支持作者

如果这个项目对你有帮助，欢迎赞赏支持：

<p align="center">
  <img src="public/sponsor.png" width="200" alt="赞赏码">
</p>

> "感谢每一份默默的支持，若愿意用赞赏表达喜爱，我会加倍珍惜 ~"

## 📫 联系方式

- 作者：luoka
- 邮箱：2803278835@qq.com
- 微信：luoka328

---

<p align="center">
  如果这个项目对你有帮助，请给个 ⭐ Star！
</p>
